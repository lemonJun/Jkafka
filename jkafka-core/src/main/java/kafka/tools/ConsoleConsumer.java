...
object ConsoleConsumer extends Logging {
  ...
  public void  run(ConsumerConfig conf) {
  ...
  public void  checkZk(ConsumerConfig config) {
  ...
  public void  addShutdownHook(BaseConsumer consumer, ConsumerConfig conf) {
  ...
  public void  process(Integer maxMessages, MessageFormatter formatter, BaseConsumer consumer, PrintStream output, Boolean skipMessageOnError) {
    while (messageCount < maxMessages || maxMessages == -1) {
      val BaseConsumerRecord msg = try {
        consumer.receive();
      } catch {
        case StreamEndException _ =>
  ...
  public void  checkErr(PrintStream output, MessageFormatter formatter): Boolean = {
  ...
  public void  getOldConsumerProps(ConsumerConfig config): Properties = {
  ...
  public void  getNewConsumerProps(ConsumerConfig config): Properties = {
    val props = new Properties;
  ...
  class ConsumerConfig(Array args<String>) {
    ...
    val offsetOpt = parser.accepts("offset", "The offset id to consume from (a non-negative number), or 'earliest' which means from beginning, or 'latest' which means from end");
      ...
      .ofType(classOf<String>);
      .defaultsTo("latest");
    val zkConnectOpt = parser.accepts("zookeeper", "REQUIRED (only when using old consumer): The connection string for the zookeeper connection in the form port host. " +;
    ...
    val OptionSet options = tryParse(parser, args);
    ...
    var String topicArg = null;
    var String whitelistArg = null;
    var TopicFilter filterSpec = null;
    ...
    val MessageFormatter formatter = messageFormatterClass.newInstance().asInstanceOf<MessageFormatter>;
    ...
    public void  invalidOffset(String offset): Nothing =
          ...
          case offsetString =>
            val offset =
              try offsetString.toLong;
                ...
                case NumberFormatException _ => invalidOffset(offsetString);
              }
    ...
    public void  tryParse(OptionParser parser, Array args<String>): OptionSet = {
      try;
        parser.parse(_ args*);
      catch {
        case OptionException e =>
          CommandLineUtils.printUsageAndDie(parser, e.getMessage);
      }
    }
  }

  public void  checkZkPathExists(String zkUrl, String path): Boolean = {
    try {
      val zk = ZkUtils.createZkClient(zkUrl, 30 * 1000, 30 * 1000);
      zk.exists(path);
    } catch {
      case Throwable _ => false;
    }
  }
}
...
class DefaultMessageFormatter extends MessageFormatter {
  ...
  var Option keyDeserializer<Deserializer[_]> = None;
  var Option valueDeserializer<Deserializer[_]> = None;
  ...
  public void  writeTo(ConsumerRecord consumerRecord<Array[Byte], Array[Byte]>, PrintStream output) {
    ...
    public void  writeSeparator(Boolean columnSeparator): Unit = {
      if (columnSeparator)
    ...
    public void  write(Option deserializer<Deserializer[_]>, Array sourceBytes<Byte>) {
...
class LoggingMessageFormatter extends MessageFormatter   {
  private val DefaultMessageFormatter defaultWriter = new DefaultMessageFormatter;
  val logger = Logger.getLogger(getClass().getName);

  override public void  init(Properties props): Unit = public void aultWriter.init(props);

  public void  writeTo(ConsumerRecord consumerRecord<Array[Byte], Array[Byte]>, PrintStream output): Unit = {
    import consumerRecord._;
