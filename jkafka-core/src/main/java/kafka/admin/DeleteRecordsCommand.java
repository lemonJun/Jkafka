...
object DeleteRecordsCommand {
  ...
  public void  parseOffsetJsonStringWithoutDedup(String jsonData): Seq<(TopicPartition, Long)> = {
  ...
  public void  execute(Array args<String>, PrintStream out): Unit = {
    ...
    if (duplicatePartitions.nonEmpty)
      throw new AdminCommandFailedException(String.format("Offset json file contains duplicate topic partitions: %s",duplicatePartitions.mkString(",")))

    out.println("Executing records delete operation");
    val Map deleteRecordsResult<TopicPartition, DeleteRecordsResult> = adminClient.deleteRecordsBefore(offsetSeq.toMap).get()
  ...
  private public void  createAdminClient(DeleteRecordsCommandOptions opts): AdminClient = {
  ...
  class DeleteRecordsCommandOptions(Array args<String>) {
    val BootstrapServerDoc = "The REQUIRED server to connect to.";
    val offsetJsonFileDoc = "The REQUIRED JSON file with offset per partition. The format to use is:\n" +;
