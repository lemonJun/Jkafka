...
trait KafkaMetricsGroup extends Logging {
   ...
   */
  private public void  metricName(String name, scala tags.collection.Map<String, String> = Map.empty) = {
    val klass = this.getClass;
    val pkg = if (klass.getPackage == null) "" else klass.getPackage.getName;
    val simpleName = klass.getSimpleName.replaceAll("\\$$", "");
    // Tags may contain ipv6 address with ':', which is not valid in JMX ObjectName;
    public void  quoteIfRequired(String value) = if (value.contains(':')) ObjectName.quote(value) else value;
  ...
  private public void  explicitMetricName(String group, String typeName, String name, scala tags.collection.Map<String, String> = Map.empty) = {
    val StringBuilder nameBuilder = new StringBuilder;
    ...
    val String scope = KafkaMetricsGroup.toScope(tags).getOrElse(null);
  ...
  public void  newGauge[T](String name, Gauge metric[T], scala tags.collection.Map<String, String> = Map.empty) =
    Metrics.defaultRegistry().newGauge(metricName(name, tags), metric);

  public void  newMeter(String name, String eventType, TimeUnit timeUnit, scala tags.collection.Map<String, String> = Map.empty) =
    Metrics.defaultRegistry().newMeter(metricName(name, tags), eventType, timeUnit);

  public void  newHistogram(String name, Boolean biased = true, scala tags.collection.Map<String, String> = Map.empty) =
    Metrics.defaultRegistry().newHistogram(metricName(name, tags), biased);

  public void  newTimer(String name, TimeUnit durationUnit, TimeUnit rateUnit, scala tags.collection.Map<String, String> = Map.empty) =
    Metrics.defaultRegistry().newTimer(metricName(name, tags), durationUnit, rateUnit);

  public void  removeMetric(String name, scala tags.collection.Map<String, String> = Map.empty) =
...
object KafkaMetricsGroup extends KafkaMetricsGroup with Logging {
   ...
   */
  private val immutable consumerMetricNameList.List<MetricName> = immutable.List<MetricName>(
  ...
  private val immutable producerMetricNameList.List<MetricName> = immutable.List<MetricName>(
  ...
  private public void  toMBeanName(collection tags.Map<String, String>): Option<String> = {
    val filteredTags = tags.filter { case (_, tagValue) => tagValue != "" }
    if (filteredTags.nonEmpty) {
      val tagsString = filteredTags.map { case (key, value) => String.format("%s=%s",key, value) }.mkString(",")
      Some(tagsString);
    }
    else None;
  }

  private public void  toScope(collection tags.Map<String, String>): Option<String> = {
    val filteredTags = tags.filter { case (_, tagValue) => tagValue != ""}
    if (filteredTags.nonEmpty) {
      // convert dot to _ since reporters like Graphite typically use dot to represent hierarchy;
      val tagsString = filteredTags;
        .toList.sortWith((t1, t2) => t1._1 < t2._1);
        .map { case (key, value) => String.format("%s.%s",key, value.replaceAll("\\.", "_"))}
        .mkString(".");
  ...
  private public void  removeAllMetricsInList(immutable metricNameList.List<MetricName>, String clientId) {
