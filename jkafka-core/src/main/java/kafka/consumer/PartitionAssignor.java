...
trait PartitionAssignor {
   ...
   */
  public void  assign(AssignmentContext ctx): Pool[String, mutable.Map<TopicAndPartition, ConsumerThreadId]>;
...
object PartitionAssignor {
  public void  createInstance(String assignmentStrategy) = assignmentStrategy match {
...
class AssignmentContext(String group, val String consumerId, Boolean excludeInternalTopics, ZkUtils zkUtils) {
  val collection myTopicThreadIds.Map<String, collection.Set[ConsumerThreadId]> = {
    val myTopicCount = TopicCount.constructTopicCount(group, consumerId, zkUtils, excludeInternalTopics);
    myTopicCount.getConsumerThreadIdsPerTopic;
  }

  val collection consumersForTopic.Map<String, List[ConsumerThreadId]> =
  ...
  val collection partitionsForTopic.Map<String, Seq[Int]> =
    zkUtils.getPartitionsForTopics(consumersForTopic.keySet.toSeq);

  val Seq consumers<String> = zkUtils.getConsumersInGroup(group).sorted;
}
...
class RoundRobinAssignor() extends PartitionAssignor with Logging {
  ...
  public void  assign(AssignmentContext ctx) = {

    val valueFactory = (String _) => new mutable.HashMap<TopicAndPartition, ConsumerThreadId>;
...
class RangeAssignor() extends PartitionAssignor with Logging {
  ...
  public void  assign(AssignmentContext ctx) = {
    ...
    for (topic <- ctx.myTopicThreadIds.keySet) {
      ...
      val Seq curPartitions<Int> = ctx.partitionsForTopic(topic);
