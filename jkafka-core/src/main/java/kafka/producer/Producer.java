...
class Producer<K,V>(val ProducerConfig config,
  ...
  private public void  recordStats(Seq messages<KeyedMessage[K,V]>) {
  ...
  private public void  asyncSend(Seq messages<KeyedMessage[K,V]>) {
    for (message <- messages) {
