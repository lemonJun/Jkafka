...
object Message {
  ...
  private val MessageHeaderSizeMap = Map (
    (Byte 0) -> (CrcLength + MagicLength + AttributesLength + KeySizeLength + ValueSizeLength),
    (Byte 1) -> (CrcLength + MagicLength + AttributesLength + TimestampLength + KeySizeLength + ValueSizeLength));
   ...
   */
  val Byte MagicValue_V0 = 0;
  val Byte MagicValue_V1 = 1;
  val Byte CurrentMagicValue = 1;

  /**
   * Specifies the mask for the compression code. 3 bits to hold the compression codec.
   * 0 is reserved to indicate no compression
   */
  val Integer CompressionCodeMask = 0x07;
  /**
   * Specifies the mask for timestamp type. 1 bit at the 4th least significant bit.
   * 0 for CreateTime, 1 for LogAppendTime
   */
  val Byte TimestampTypeMask = 0x08;
  val Integer TimestampTypeAttributeBitOffset = 3;

  /**
   * Compression code for uncompressed messages
   */
  val Integer NoCompression = 0;

  /**
   * To indicate timestamp is not defined so "magic" value 0 will be used.
   */
  val Long NoTimestamp = -1;

  /**
   * Give the header size difference between different message versions.
   */
  public void  headerSizeDiff(Byte fromMagicValue, Byte toMagicValue) : Integer =
    MessageHeaderSizeMap(toMagicValue) - MessageHeaderSizeMap(fromMagicValue);


  public void  fromRecord(LegacyRecord record): Message = {
    val Option wrapperTimestamp<Long> = if (record.wrapperRecordTimestamp == null) None else Some(record.wrapperRecordTimestamp)
 ...
 */
class Message(val ByteBuffer buffer,
              private val Option wrapperMessageTimestamp<Long> = None,
              private val Option wrapperMessageTimestampType<TimestampType> = None) {
  ;
  import kafka.message.Message._;

  private<message> public void  LegacyRecord asRecord = wrapperMessageTimestamp match {
   ...
   */
  public void  this(Array bytes<Byte>, ;
           Array key<Byte>,
           Long timestamp,
           TimestampType timestampType,
           CompressionCodec codec, ;
           Integer payloadOffset, ;
           Integer payloadSize,
           Byte magicValue) = {
    ...
    val Byte attributes = LegacyRecord.computeAttributes(magicValue, CompressionType.forId(codec.codec), timestampType)
  ...
  public void  this(Array bytes<Byte>, Array key<Byte>, Long timestamp, TimestampType timestampType, CompressionCodec codec, Byte magicValue) =
  ...
  public void  this(Array bytes<Byte>, Array key<Byte>, Long timestamp, CompressionCodec codec, Byte magicValue) =
  ...
  ;
  public void  this(Array bytes<Byte>, Long timestamp, CompressionCodec codec, Byte magicValue) =
    this(bytes = bytes, key = null, timestamp = timestamp, codec = codec, magicValue = magicValue);
  ;
  public void  this(Array bytes<Byte>, Array key<Byte>, Long timestamp, Byte magicValue) =
    this(bytes = bytes, key = key, timestamp = timestamp, codec = NoCompressionCodec, magicValue = magicValue);
    ;
  public void  this(Array bytes<Byte>, Long timestamp, Byte magicValue) =
   ...
   */
  public void  Integer size = buffer.limit;

  /**
   * The position where the key size is stored.
   */
  private public void  keySizeOffset = {
   ...
   */
  public void  Integer keySize = buffer.getInt(keySizeOffset);
  ;
  /**
   * Does the message have a key?
   */
  public void  Boolean hasKey = keySize >= 0;
  ;
  /**
   * The position where the payload size is stored
   */
  private public void  payloadSizeOffset = {
   ...
   */
  public void  Integer payloadSize = buffer.getInt(payloadSizeOffset);
  ;
  /**
   * Is the payload of this message null
   */
  public void  Boolean isNull = payloadSize < 0;
  ;
  /**
   * The magic version of this message
   */
  public void  Byte magic = buffer.get(MagicOffset);
  ;
  /**
   * The attributes stored with this message
   */
  public void  Byte attributes = buffer.get(AttributesOffset);
   ...
   */
  public void  timestampType = LegacyRecord.timestampType(magic, wrapperMessageTimestampType.orNull, attributes);

  /**
   * The compression codec used with this message
   */
  public void  CompressionCodec compressionCodec = ;
   ...
   */
  public void  ByteBuffer payload = sliceDelimited(payloadSizeOffset);
  ;
  /**
   * A ByteBuffer containing the message key
   */
  public void  ByteBuffer key = sliceDelimited(keySizeOffset);

  /**
   * Read a size-delimited byte buffer starting at the given offset
   */
  private public void  sliceDelimited Integer start): ByteBuffer = {
   ...
   */
  private public void  validateTimestampAndMagicValue(Long timestamp, Byte magic) {
  ...
  override public void  equals(Any any): Boolean = {
    any match {
      case Message that => this.buffer.equals(that.buffer);
      case _ => false;
    }
  }
  ;
