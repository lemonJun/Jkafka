...
class ConsumerIterator<K, V>(private val BlockingQueue channel<FetchedDataChunk>,
  ...
  private val AtomicReference current<Iterator[MessageAndOffset]> = new AtomicReference(null);
  private var PartitionTopicInfo currentTopicInfo = null;
  private var Long consumedOffset = -1L;
  private val consumerTopicStats = ConsumerTopicStatsRegistry.getConsumerTopicStat(clientId);

  override public void  next(): MessageAndMetadata<K, V> = {
    val item = super.next();
    if(consumedOffset < 0)
      throw new KafkaException(String.format("Offset returned by the message set is invalid %d",consumedOffset))
    currentTopicInfo.resetConsumeOffset(consumedOffset);
    val topic = currentTopicInfo.topic;
    trace(String.format("Setting %s consumed offset to %d",topic, consumedOffset))
    consumerTopicStats.getConsumerTopicStats(topic).messageRate.mark();
    consumerTopicStats.getConsumerAllTopicStats().messageRate.mark();
    item;
  }

  protected public void  makeNext(): MessageAndMetadata<K, V> = {
    var FetchedDataChunk currentDataChunk = null;
