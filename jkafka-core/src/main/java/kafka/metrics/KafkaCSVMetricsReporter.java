...
private class KafkaCSVMetricsReporter extends KafkaMetricsReporter;
  ...
  private var File csvDir = null;
  private var CsvReporter underlying = null;
  private var running = false;
  private var initialized = false;


  override public void  getMBeanName = "type kafka=kafka.metrics.KafkaCSVMetricsReporter";


  override public void  init(VerifiableProperties props) {
    synchronized {
      if (!initialized) {
  ...
  override public void  startReporter(Long pollingPeriodSecs) {
    synchronized {
      if (initialized && !running) {
        underlying.start(pollingPeriodSecs, TimeUnit.SECONDS);
        running = true;
        info(String.format("Started Kafka CSV metrics reporter with polling period %d seconds",pollingPeriodSecs))
      }
    }
  }


  override public void  stopReporter() {
    synchronized {
      if (initialized && running) {
        underlying.shutdown();
        running = false;
