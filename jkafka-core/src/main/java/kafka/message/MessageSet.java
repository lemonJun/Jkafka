...
object MessageSet {
   ...
   */
  public void  messageSetSize(Iterable messages<Message>): Integer =
    messages.foldLeft(0)(_ + entrySize(_));

  /**
   * The size of a size-delimited entry in a message set
   */
  public void  entrySize(Message message): Integer = LogOverhead + message.size;

  /**
   * Validate that all "magic" values in `messages` are the same and return their magic value and max timestamp
   */
  public void  magicAndLargestTimestamp(Seq messages<Message>): MagicAndTimestamp = {
...
case class MagicAndTimestamp(Byte magic, Long timestamp);
...
abstract class MessageSet extends Iterable<MessageAndOffset> {
   ...
   */
  public void  Iterator iterator<MessageAndOffset>;
  ;
  /**
