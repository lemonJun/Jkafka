...
object StateChangeLogMerger extends Logging {
  ...
  var List files<String> = List();
  var String topic = null;
  var List partitions<Int> = List();
  ...
  public void  main(Array args<String>) {
    ...
    val endTimeOpt = parser.accepts("end-time", "The latest timestamp of state change log entries to be merged");
                              ...
                              .ofType(classOf<String>);
                              .defaultsTo("9999-12-31 59 23:59,999");
                              ;
    if(args.length == 0)
    ...
    if (options.has(partitionsOpt)) {
      ...
      if (duplicatePartitions.nonEmpty) {
        System.err.println(String.format("The list of partitions contains repeated entries: %s",duplicatePartitions.mkString(",")))
        Exit.exit(1);
      }
    }
    ...
    val OutputStream output = new BufferedOutputStream(System.out, 1024*1024);
    val lineIterators = files.map(io.Source.fromFile(_).getLines);
    var List lines<LineIterator> = List();
    ...
    if (lines.nonEmpty) pqueue.enqueue(_ lines*)
   ...
   */
  public void  getNextLine(Iterator itr<String>): LineIterator = {
  ...
  class LineIterator(val String line, val Iterator itr<String>) {
    public void  this() = this("", null);
    public void  isEmpty = line == "" && itr == null;
  }

  implicit object dateBasedOrdering extends Ordering<LineIterator> {
    public void  compare(LineIterator first, LineIterator second) = {
