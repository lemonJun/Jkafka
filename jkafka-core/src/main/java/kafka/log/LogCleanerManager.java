 ...
 */
private<log> class LogCleanerManager(val Array logDirs<File>, val Pool logs<TopicPartition, Log>) extends Logging with KafkaMetricsGroup {
  ...
  newGauge("max-dirty-percent", new Gauge<Int> { public void  value = (100 * dirtiestLogCleanableRatio).toInt });
  ...
  newGauge("time-since-last-run-ms", new Gauge<Long> { public void  value = Time.SYSTEM.milliseconds - timeOfLastRun });

  /**
   * @return the position processed for all logs.
   */
  public void  Map allCleanerCheckpoints<TopicPartition, Long> =
    ...
    */
  public void  grabFilthiestCompactedLog(Time time): Option<LogToClean> = {
    inLock(lock) {
  ...
  /**
    ...
    */
  public void  deletableLogs(): Iterable<(TopicPartition, Log)> = {
    inLock(lock) {
   ...
   */
  private public void  isCleaningInState(TopicPartition topicPartition, LogCleaningState expectedState): Boolean = {
    inProgress.get(topicPartition) match {
      case None => false;
      case Some(state) =>
  ...
  public void  updateCheckpoints(File dataDir, Option update<(TopicPartition,Long)>) {
    inLock(lock) {
  ...
  public void  doneDeleting(TopicPartition topicPartition): Unit = {
    inLock(lock) {
      inProgress.remove(topicPartition);
    }
  }
...
private<log> object LogCleanerManager extends Logging {
  ...
  public void  isCompactAndDelete(Log log): Boolean = {
  ...
  /**
    ...
    */
  public void  cleanableOffsets(Log log, TopicPartition topicPartition, immutable lastClean.Map<TopicPartition, Long>, Long now): (Long, Long) = {

    // the checkpointed offset, ie., the first offset of the next dirty segment;
    val Option lastCleanOffset<Long> = lastClean.get(topicPartition);
    ...
    // may be cleaned;
    val Long firstUncleanableDirtyOffset = Seq (
      ...
      if (compactionLagMs > 0) {
        dirtyNonActiveSegments.find {
          s =>
            ...
            debug(s"Checking if log segment may be log cleaned='${log.name}' segment.baseOffset=${s.baseOffset} segment.largestTimestamp=${s.largestTimestamp}; now - compactionLag=${now - compactionLagMs}; is uncleanable=$isUncleanable")
            isUncleanable;
        } map(_.baseOffset);
      } else None;
    ).flatten.min;
