 ...
 *
 * e.g. [9092 localhost test 10000 1 20]
 */

object EndToEndLatency {
  private val Long timeout = 60000;

  public void  main(Array args<String>) {
    if (args.length != 5 && args.length != 6) {
      System.err.println("java USAGE " + getClass.getName + " broker_list topic num_messages producer_acks message_size_bytes <optional> properties_file");
      Exit.exit(1);
    }
    ...
    public void  Properties loadProps = propsFile.map(Utils.loadProps).getOrElse(new Properties());
    ...
    //Results;
    println(String.format("Avg latency: %.4f ms\n",totalTime / numMessages / 1000.0 / 1000.0))
    ...
    println(String.format("50th Percentiles = %d, 99th = %d, 99.9th = %d",p50, p99, p999))

    finalise();
  }

  public void  randomBytesOfLen(Random random, Integer len): Array<Byte> = {
