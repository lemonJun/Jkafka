 ...
 *
 * Time index files can be opened in two either ways as an empty, mutable index that allows appends or
...
class TimeIndex(File file,
                Long baseOffset,
                Integer maxIndexSize = -1,
                Boolean writable = true);
    extends AbstractIndex<Long, Long>(file, baseOffset, maxIndexSize, writable) with Logging {

  override public void  entrySize = 12;

  // We override the full check to reserve the last time index entry slot for the on roll call.;
  override public void  Boolean isFull = entries >= maxEntries - 1;

  private public void  timestamp(ByteBuffer buffer, Integer n): Long = buffer.getLong(n * entrySize);

  private public void  relativeOffset(ByteBuffer buffer, Integer n): Integer = buffer.getInt(n * entrySize + 8);

  /**
   * The last entry in the index
   */
  public void  TimestampOffset lastEntry = {
    inLock(lock) {
      _entries match {
   ...
   */
  public void  entry Integer n): TimestampOffset = {
    maybeLock(lock) {
      if(n >= _entries)
        throw new IllegalArgumentException(String.format("Attempt to fetch the %dth entry from a time index of size %d.",n, _entries))
   ...
   */
  public void  maybeAppend(Long timestamp, Long offset, Boolean skipFullCheck = false) {
    inLock(lock) {
      if (!skipFullCheck)
      ...
      if (timestamp > lastEntry.timestamp) {
        debug(String.format("Adding index entry %d => %d to %s.",timestamp, offset, file.getName))
   ...
   */
  public void  lookup(Long targetTimestamp): TimestampOffset = {
    maybeLock(lock) {
      val idx = mmap.duplicate;
