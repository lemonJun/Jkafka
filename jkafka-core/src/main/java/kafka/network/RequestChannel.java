...
object RequestChannel extends Logging {
  ...
  private public void  ByteBuffer shutdownReceive = {
  ...
  case class Session(KafkaPrincipal principal, InetAddress clientAddress) {
    val sanitizedUser = QuotaId.sanitize(principal.getName);
  }

  case class Request Integer processor, String connectionId, Session session, private var ByteBuffer buffer,
                     Long startTimeNanos, ListenerName listenerName, SecurityProtocol securityProtocol) {
    ...
    @volatile var Option recordNetworkThreadTimeCallback[Long => Unit] = None

    val requestId = buffer.getShort();

    // this TODO will be removed once we remove support for v0 of ControlledShutdownRequest (which;
    // depends on a non-standard request header);
    val RequestOrResponse requestObj = if (requestId == ApiKeys.CONTROLLED_SHUTDOWN_KEY.id)
    ...
    val RequestHeader header =
      if (requestObj == null) {
        ...
        null;
    val RequestAndSize bodyAndSize =
      if (requestObj == null)
        try {
    ...
    public void  requestDesc(Boolean details): String = {
      if (requestObj != null)
    ...
    public void  body[T <: AbstractRequest](implicit ClassTag classTag[T], NotNothing nn[T]): T = {
    ...
    trace(String.format("Processor %d received request : %s",processor, requestDesc(true)))

    public void  requestThreadTimeNanos = {
      if (apiLocalCompleteTimeNanos == -1L) apiLocalCompleteTimeNanos = Time.SYSTEM.nanoseconds;
      math.max(apiLocalCompleteTimeNanos - requestDequeueTimeNanos, 0L);
    }

    public void  updateRequestMetrics(Long networkThreadTimeNanos) {
      ...
      public void  nanosToMs(Long nanos) = math.max(TimeUnit.NANOSECONDS.toMillis(nanos), 0);
      ...
      if (requestLogger.isDebugEnabled) {
        ...
        public void  nanosToMs(Long nanos) = TimeUnit.NANOSECONDS.toMicros(math.max(nanos, 0)).toDouble / TimeUnit.MILLISECONDS.toMicros(1);
  ...
  object Response {
    ...
    public void  apply(Request request, Send responseSend): Response = {
    ...
    public void  apply(Request request, AbstractResponse response): Response = {
      require(request != null, "request should be non null");
  ...
  case class Response(Request request, Option responseSend<Send>, ResponseAction responseAction) {
    ...
    public void  Integer processor = request.processor;
  }
...
class RequestChannel(val Integer numProcessors, val Integer queueSize) extends KafkaMetricsGroup {
  private var List responseListeners[(Int) => Unit] = Nil;
  ...
  newGauge(
    "RequestQueueSize",
    new Gauge<Int> {
      public void  value = requestQueue.size;
    }
  );

  newGauge("ResponseQueueSize", new Gauge<Int>{
    public void  value = responseQueues.foldLeft(0) {(total, q) => total + q.size()}
  });
