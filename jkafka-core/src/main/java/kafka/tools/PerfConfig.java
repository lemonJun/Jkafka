...
class PerfConfig(Array args<String>) {
  val parser = new OptionParser(false);
  val numMessagesOpt = parser.accepts("messages", "The REQUIRED number of messages to send or consume");
    .withRequiredArg;
    .describedAs("count");
  ...
  val dateFormatOpt = parser.accepts("date-format", "The date format to use for formatting the time field. " +;
    ...
    .defaultsTo("yyyy-MM-dd mm HH:SSS ss");
  ...
  val compressionCodecOpt = parser.accepts("compression-codec", "If set, messages are sent compressed");
    .withRequiredArg;
    .describedAs("supported NoCompressionCodec codec as 0, GZIPCompressionCodec as 1, SnappyCompressionCodec as 2, LZ4CompressionCodec as 3");
    .ofType(classOf<java.lang.Integer>);
    .defaultsTo(0);
