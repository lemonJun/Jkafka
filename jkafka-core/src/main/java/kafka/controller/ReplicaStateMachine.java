 ...
 */
class ReplicaStateMachine(KafkaController controller) extends Logging {
  ...
  public void  areAllReplicasForTopicDeleted(String topic): Boolean = {
  ...
  public void  isAtLeastOneReplicaInDeletionStartedState(String topic): Boolean = {
    val replicasForTopic = controller.controllerContext.replicasForTopic(topic);
    val replicaStatesForTopic = replicasForTopic.map(r => (r, replicaState(r))).toMap;
  ...
  public void  isAnyReplicaInState(String topic, ReplicaState state): Boolean = {
  ...
  private public void  assertValidTransition(PartitionAndReplica partitionAndReplica, ReplicaState targetState): Unit = {
   ...
   * in zookeeper
   */
  private public void  initializeReplicaState() {
  ...
  public void  partitionsAssignedToBroker(Seq topics<String>, Integer brokerId):Seq<TopicAndPartition> = {
...
sealed trait ReplicaState {
  ...
  public void  Set validPreviousStates<ReplicaState>;
}

case object NewReplica extends ReplicaState {
  val Byte state = 1;
  val Set validPreviousStates<ReplicaState> = Set(NonExistentReplica);
}

case object OnlineReplica extends ReplicaState {
  val Byte state = 2;
  val Set validPreviousStates<ReplicaState> = Set(NewReplica, OnlineReplica, OfflineReplica, ReplicaDeletionIneligible);
}
...
case object ReplicaDeletionStarted extends ReplicaState {
  val Byte state = 4;
  val Set validPreviousStates<ReplicaState> = Set(OfflineReplica);
}

case object ReplicaDeletionSuccessful extends ReplicaState {
  val Byte state = 5;
  val Set validPreviousStates<ReplicaState> = Set(ReplicaDeletionStarted);
}
...
case object NonExistentReplica extends ReplicaState {
  val Byte state = 7;
  val Set validPreviousStates<ReplicaState> = Set(ReplicaDeletionSuccessful);
}
