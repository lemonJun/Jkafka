...
object OffsetResponse {
  ...
  public void  readFrom(ByteBuffer buffer): OffsetResponse = {
    ...
    OffsetResponse(correlationId, Map(_ pairs*));
  }

}


case class PartitionOffsetsResponse(Errors error, Seq offsets<Long>) {
...
case class OffsetResponse Integer correlationId,
                          Map partitionErrorAndOffsets<TopicAndPartition, PartitionOffsetsResponse>);
    extends RequestOrResponse() {

  lazy val offsetsGroupedByTopic = partitionErrorAndOffsets.groupBy(_._1.topic);

  public void  hasError = partitionErrorAndOffsets.values.exists(_.error != Errors.NONE);
