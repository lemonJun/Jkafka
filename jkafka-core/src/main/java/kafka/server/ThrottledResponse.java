 ...
 */
private<server> class ThrottledResponse(val Time time, val Integer throttleTimeMs, Integer callback => Unit) extends Delayed {
  val endTime = time.milliseconds + throttleTimeMs;

  public void  execute() = callback(throttleTimeMs);

  override public void  getDelay(TimeUnit unit): Long = {
    unit.convert(endTime - time.milliseconds, TimeUnit.MILLISECONDS);
  }

  override public void  compareTo(Delayed d): Integer = {
