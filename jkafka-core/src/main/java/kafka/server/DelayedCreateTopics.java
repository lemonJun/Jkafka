...
/**
  ...
  *
  * local TODO state doesn't count, need to know state of all relevant brokers
  *
  */
case class CreateTopicMetadata(String topic, Map replicaAssignments<Int, Seq[Int]>, ApiError error);

/**
  * A delayed create topics operation that can be created by the admin manager and watched
  * in the topic purgatory
  */
class DelayedCreateTopics(Long delayMs,
                          Seq createMetadata<CreateTopicMetadata>,
  ...
  override public void  onExpiration(): Unit = { }

  private public void  missingLeaderCount(String topic, Set partitions<Int>): Integer = {
    partitions.foldLeft(0) { case (counter, partition) =>
      if (isMissingLeader(topic, partition)) counter + 1 else counter;
    }
  }

  private public void  isMissingLeader(String topic, Integer partition): Boolean = {
