...
class TransactionMarkerRequestCompletionHandler Integer brokerId,
                                                TransactionStateManager txnStateManager,
                                                TransactionMarkerChannelManager txnMarkerChannelManager,
                                                java txnIdAndMarkerEntries.util.List<TxnIdAndMarkerEntry>) extends RequestCompletionHandler with Logging {

  this.logIdent = "[Transaction Marker Request Completion Handler " + brokerId + "]: ";

  override public void  onComplete(ClientResponse response): Unit = {
      ...
      for (TxnIdAndMarkerEntry txnIdAndMarker <- txnIdAndMarkerEntries) {
        ...
        txnStateManager.getTransactionState(transactionalId) match {
          ...
          case Right(Some(epochAndMetadata)) =>
            ...
            val mutable retryPartitions.Set<TopicPartition> = mutable.Set.empty<TopicPartition>;
            var Boolean abortSending = false;
            ...
            if (epochAndMetadata.coordinatorEpoch != txnMarker.coordinatorEpoch) {
              ...
              txnMetadata synchronized {
                for ((TopicPartition topicPartition, Errors error) <- errors) {
                  error match {
                    case Errors.NONE =>
