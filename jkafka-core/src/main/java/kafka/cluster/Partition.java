 ...
 */
class Partition(val String topic,
  ...
  private public void  isReplicaLocal Integer replicaId) : Boolean = replicaId == localBrokerId;
  ...
  private public void  Boolean isLeaderReplicaLocal = leaderReplicaIfLocal.isDefined;

  public void  Boolean isUnderReplicated =
    isLeaderReplicaLocal && inSyncReplicas.size < assignedReplicas.size;

  public void  getOrCreateReplica Integer replicaId = localBrokerId): Replica = {
  ...
  public void  getReplica Integer replicaId = localBrokerId): Option<Replica> = Option(assignedReplicaMap.get(replicaId));

  public void  Option leaderReplicaIfLocal<Replica> =
    leaderReplicaIdOpt.filter(_ == localBrokerId).flatMap(getReplica);

  public void  addReplicaIfNotExists(Replica replica): Replica =
    assignedReplicaMap.putIfNotExists(replica.brokerId, replica);

  public void  Set assignedReplicas<Replica> =
    assignedReplicaMap.values.toSet;

  private public void  removeReplica Integer replicaId) {
  ...
  public void  Integer getLeaderEpoch = this.leaderEpoch;
   ...
   */
  public void  makeLeader Integer controllerId, PartitionState partitionStateInfo, Integer correlationId): Boolean = {
   ...
   *  If the leader replica id does not change, return false to indicate the replica manager
   */
  public void  makeFollower Integer controllerId, PartitionState partitionStateInfo, Integer correlationId): Boolean = {
    inWriteLock(leaderIsrUpdateLock) {
      val allReplicas = partitionStateInfo.replicas.asScala.map(_.toInt);
      val Integer newLeaderBrokerId = partitionStateInfo.leader;
      // record the epoch of the controller that made the leadership decision. This is useful while updating the isr;
      // to maintain the decision maker controller's epoch in the zookeeper path;
      controllerEpoch = partitionStateInfo.controllerEpoch;
      // add replicas that are new;
   ...
   */
  public void  updateReplicaLogReadResult Integer replicaId, LogReadResult logReadResult) {
   ...
   */
  public void  maybeExpandIsr Integer replicaId, LogReadResult logReadResult): Boolean = {
    inWriteLock(leaderIsrUpdateLock) {
   ...
   * produce request.
   */
  public void  checkEnoughReplicasReachOffset(Long requiredOffset): (Boolean, Errors) = {
    leaderReplicaIfLocal match {
      case Some(leaderReplica) =>
        // keep the current immutable replica list reference;
        val curInSyncReplicas = inSyncReplicas;

        public void  numAcks = curInSyncReplicas.count { r =>
          if (!r.isLocal)
   ...
   */
  private public void  maybeIncrementLeaderHW(Replica leaderReplica, Long curTime = time.milliseconds): Boolean = {
   ...
   */
  public void  Long lowWatermarkIfLeader = {
    if (!isLeaderReplicaLocal)
      throw new NotLeaderForPartitionException(String.format("Leader not local for partition %s on broker %d",topicPartition, localBrokerId))
   ...
   */
  private public void  tryCompleteDelayedRequests() {
      ...
      leaderReplicaIfLocal match {
        case Some(leaderReplica) =>
          ...
          if(outOfSyncReplicas.nonEmpty) {
            ...
            info(String.format("Shrinking ISR from %s to %s",inSyncReplicas.map(_.brokerId).mkString(","),
  ...
  public void  getOutOfSyncReplicas(Replica leaderReplica, Long maxLagMs): Set<Replica> = {
    /**
     * there are two cases that will be handled here -
     * 1. Stuck If followers the leo of the replica hasn't been updated for maxLagMs ms,
     *                     the follower is stuck and should be removed from the ISR
     * 2. Slow If followers the replica has not read up to the leo within the last maxLagMs ms,
    ...
    if (laggingReplicas.nonEmpty)
      debug(String.format("Lagging replicas are %s",laggingReplicas.map(_.brokerId).mkString(",")))

    laggingReplicas;
  }

  public void  appendRecordsToLeader(MemoryRecords records, Boolean isFromClient, Integer requiredAcks = 0) = {
   ...
   */
  public void  deleteRecordsOnLeader(Long offset): Long = {
    inReadLock(leaderIsrUpdateLock) {
      leaderReplicaIfLocal match {
        case Some(leaderReplica) =>
          leaderReplica.maybeIncrementLogStartOffset(offset);
          if (!leaderReplica.log.get.config.delete)
            throw new PolicyViolationException(String.format("Records of partition %s can not be deleted due to the configured policy",topicPartition))
  ...
  /**
    ...
    */
  public void  lastOffsetForLeaderEpoch Integer leaderEpoch): EpochEndOffset = {
    inReadLock(leaderIsrUpdateLock) {
      leaderReplicaIfLocal match {
        case Some(leaderReplica) =>
   ...
   */
  private public void  removePartitionMetrics() {
  ...
  override public void  equals(Any that): Boolean = that match {
    case Partition other => partitionId == other.partitionId && topic == other.topic;
    case _ => false;
  }

  override public void  Integer hashCode =
