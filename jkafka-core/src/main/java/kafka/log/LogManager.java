 ...
 */
@threadsafe
class LogManager(val Array logDirs<File>,
                 val Map topicConfigs<String, LogConfig>, // note that this doesn't get updated after creation;
                 val LogConfig defaultConfig,
                 val CleanerConfig cleanerConfig,
                 Integer ioThreads,
                 val Long flushCheckMs,
                 val Long flushRecoveryOffsetCheckpointMs,
                 val Long flushStartOffsetCheckpointMs,
                 val Long retentionCheckMs,
                 val Integer maxPidExpirationMs,
                 Scheduler scheduler,
   ...
   * </ol>
   */
  private public void  createAndValidateLogDirs(Seq dirs<File>) {
   ...
   */
  private public void  lockLogDirs(Seq dirs<File]): Seq[FileLock> = {
    dirs.map { dir =>
   ...
   */
  private public void  loadLogs(): Unit = {
    info("Loading logs.");
   ...
   */
  public void  truncateTo(Map partitionOffsets<TopicPartition, Long>) {
   ...
   */
  private public void  checkpointLogRecoveryOffsetsInDir(File dir): Unit = {
   ...
   */
  private public void  checkpointLogStartOffsetsInDir(File dir): Unit = {
   ...
   */
  public void  getLog(TopicPartition topicPartition): Option<Log> = Option(logs.get(topicPartition));

  /**
   * Create a log for the given topic and the given partition
   * If the log already exists, just return a copy of the existing log
   */
  public void  createLog(TopicPartition topicPartition, LogConfig config): Log = {
   ...
   */
  private public void  deleteLogs(): Unit = {
    try {
      var failed = 0;
  ...
  public void  asyncDelete(TopicPartition topicPartition) = {
    val Log removedLog = logCreationOrDeletionLock synchronized {
   ...
   */
  private public void  nextLogDir(): File = {
    if(logDirs.size == 1) {
      logDirs(0);
    } else {
   ...
   */
  public void  Map logsByTopicPartition<TopicPartition, Log> = logs.toMap;

  /**
   * Map of log dir to logs by topic and partitions in that dir
   */
  private public void  logsByDir = {
   ...
   */
  private public void  flushDirtyLogs() = {
