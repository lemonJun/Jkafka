...
class ReplicaFetcherThread(String name,
                           Integer fetcherId,
                           ...
                           Metrics metrics,
                           Time time,
                           ReplicationQuotaManager quota,
                           Option leaderEndpointBlockingSend<BlockingSend> = None);
  ...
  private val Short fetchRequestVersion =
  ...
  private val Boolean shouldSendLeaderEpochRequest = brokerConfig.interBrokerProtocolVersion >= KAFKA_0_11_0_IV2;

  private public void  epochCache(TopicPartition tp): LeaderEpochCache =  replicaMgr.getReplica(tp).get.epochs.get;
  ...
  public void  processPartitionData(TopicPartition topicPartition, Long fetchOffset, PartitionData partitionData) {
    try {
      ...
      if (fetchOffset != replica.logEndOffset.messageOffset)
        throw new RuntimeException(String.format("Offset mismatch for partition %fetched s offset = %d, log end offset = %d.",topicPartition, fetchOffset, replica.logEndOffset.messageOffset))
  ...
  public void  maybeWarnIfOversizedRecords(MemoryRecords records, TopicPartition topicPartition): Unit = {
  ...
  public void  handleOffsetOutOfRange(TopicPartition topicPartition): Long = {
     ...
     */
    val Long leaderEndOffset = earliestOrLatestOffset(topicPartition, ListOffsetRequest.LATEST_TIMESTAMP);
    ...
    if (leaderEndOffset < replica.logEndOffset.messageOffset) {
       ...
       *
       */
      val Long leaderStartOffset = earliestOrLatestOffset(topicPartition, ListOffsetRequest.EARLIEST_TIMESTAMP);
  ...
  private public void  earliestOrLatestOffset(TopicPartition topicPartition, Long earliestOrLatest): Long = {
    val requestBuilder = if (brokerConfig.interBrokerProtocolVersion >= KAFKA_0_10_1_IV2) {
        val partitions = Map(topicPartition -> (java earliestOrLatest.lang.Long));
  ...
  override public void  buildFetchRequest(Seq partitionMap<(TopicPartition, PartitionFetchState)>): FetchRequest = {
  ...
  /**
    ...
    */
  override public void  maybeTruncate(Map fetchedEpochs<TopicPartition, EpochEndOffset>): Map<TopicPartition, Long> = {
  ...
  override public void  buildLeaderEpochRequest(Seq allPartitions<(TopicPartition, PartitionFetchState)>): Map<TopicPartition, Int> = {
  ...
  override public void  fetchEpochsFromLeader(Map partitions<TopicPartition, Int>): Map<TopicPartition, EpochEndOffset> = {
    var Map result<TopicPartition, EpochEndOffset> = null;
  ...
  private public void  logEndOffset(Replica replica, EpochEndOffset epochOffset): Long = {
  ...
  private public void  highWatermark(Replica replica, EpochEndOffset epochOffset): Long = {
   ...
   */
  private public void  shouldFollowerThrottle(ReplicaQuota quota, TopicPartition topicPartition): Boolean = {
...
object ReplicaFetcherThread {
  ...
  private<server> class FetchRequest(val JFetchRequest underlying.Builder) extends AbstractFetcherThread.FetchRequest {
    public void  Boolean isEmpty = underlying.fetchData().isEmpty;
  ...
  private<server> class PartitionData(val FetchResponse underlying.PartitionData) extends AbstractFetcherThread.PartitionData {
    ...
    public void  Long highWatermark = underlying.highWatermark;

    public void  Long logStartOffset = underlying.logStartOffset;

    public void  Option exception<Throwable> = error match {
