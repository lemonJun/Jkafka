...
object ConfigCommand extends Config {
  ...
  private<admin> public void  alterConfig(ZkUtils zkUtils, ConfigCommandOptions opts, AdminUtilities utils = AdminUtils) {
  ...
  private public void  preProcessScramCredentials(Properties configsToBeAdded) {
    public void  scramCredential(ScramMechanism mechanism, String credentialStr): String = {
  ...
  private public void  describeConfig(ZkUtils zkUtils, ConfigCommandOptions opts) {
  ...
  private<admin> public void  parseConfigsToBeAdded(ConfigCommandOptions opts): Properties = {
    ...
    if (opts.options.has(opts.addConfig)) {
      ...
      require(configsToBeAdded.forall(config => config.length == 2), "Invalid entity all config configs to be added must be in the format \"key=val\".")
  ...
  private<admin> public void  parseConfigsToBeDeleted(ConfigCommandOptions opts): Seq<String> = {
  ...
  case class Entity(String entityType, Option sanitizedName<String>) {
  ...
  case class ConfigEntity(Entity root, Option child<Entity>) {
    val fullSanitizedName = root.sanitizedName.getOrElse("") + child.map(s => "/" + s.entityPath).getOrElse("");

    public void  getAllEntities(ZkUtils zkUtils) : Seq<ConfigEntity> = {
  ...
  private<admin> public void  parseEntity(ConfigCommandOptions opts): ConfigEntity = {
  ...
  private public void  parseQuotaEntity(ConfigCommandOptions opts): ConfigEntity = {
    ...
    public void  sanitizeName(String entityType, String name) = {
      if (name.isEmpty)
  ...
  class ConfigCommandOptions(Array args<String>) {
    ...
    val Set allOpts<OptionSpec[_]> = Set(alterOpt, describeOpt, entityType, entityName, addConfig, deleteConfig, helpOpt);
    ...
    public void  checkArgs() {
      ...
      if(options.has(alterOpt)) {
        ...
        val Boolean isAddConfigPresent = options.has(addConfig);
        val Boolean isDeleteConfigPresent = options.has(deleteConfig);
