...
public abstract class BaseMessageSetTestCases {
    ...
    public void testSizeInBytes() {
        assertEquals("Empty message set should have 0 bytes.", 0, createMessageSet(Lists.<Message> newArrayList()).sizeInBytes());
        assertEquals("Predicted size should equal actual size.", MessageSets.messageSetSize(messages), createMessageSet(messages).sizeInBytes());
    }
