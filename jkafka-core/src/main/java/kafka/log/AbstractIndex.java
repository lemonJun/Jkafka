 ...
 */
abstract class AbstractIndex<K, V>(@volatile var File file, val Long baseOffset, val Integer maxIndexSize = -1, val Boolean writable)
    extends Logging {

  protected public void  Integer entrySize;

  protected val lock = new ReentrantLock;

  @volatile
  protected var MappedByteBuffer mmap = {
   ...
   */
  public void  Boolean isFull = _entries >= _maxEntries;

  public void  Integer maxEntries = _maxEntries;

  public void  Integer entries = _entries;
   ...
   */
  public void  sizeInBytes = entrySize * _entries;
   ...
   */
  protected public void  forceUnmap(MappedByteBuffer m) {
    try {
      m match {
        case DirectBuffer buffer =>
      ...
      case Throwable t => error("Error when freeing index buffer", t);
    }
  }
   ...
   */
  protected public void  maybeLock[T](Lock lock)(fun: => T): T = {
    if (OperatingSystem.IS_WINDOWS)
      lock.lock();
    try fun;
    finally {
   ...
   */
  protected public void  parseEntry(ByteBuffer buffer, Integer n): IndexEntry;
   ...
   */
  protected public void  largestLowerBoundSlotFor(ByteBuffer idx, Long target, IndexSearchEntity searchEntity): Integer =
   ...
   */
  protected public void  smallestUpperBoundSlotFor(ByteBuffer idx, Long target, IndexSearchEntity searchEntity): Integer =
    indexSlotRangeFor(idx, target, searchEntity)._2;

  /**
   * Lookup lower and upper bounds for the given target.
   */
  private public void  indexSlotRangeFor(ByteBuffer idx, Long target, IndexSearchEntity searchEntity): (Int, Int) = {
  ...
  private public void  compareIndexEntry(IndexEntry indexEntry, Long target, IndexSearchEntity searchEntity): Integer = {
    searchEntity match {
   ...
   */
  private public void  roundDownToExactMultiple Integer number, Integer factor) = factor * (number / factor);
