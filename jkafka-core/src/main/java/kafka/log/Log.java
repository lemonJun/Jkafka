 ...
 */
case class LogAppendInfo(var Long firstOffset,
                         var Long lastOffset,
                         var Long maxTimestamp,
                         var Long offsetOfMaxTimestamp,
                         var Long logAppendTime,
                         CompressionCodec sourceCodec,
                         CompressionCodec targetCodec,
                         Integer shallowCount,
                         Integer validBytes,
 ...
 */
case class CompletedTxn(Long producerId, Long firstOffset, Long lastOffset, Boolean isAborted) {
 ...
 */
@threadsafe
class Log(@volatile var File dir,
          @volatile var LogConfig config,
          @volatile var Long logStartOffset,
          @volatile var Long recoveryPoint,
          Scheduler scheduler,
          BrokerTopicStats brokerTopicStats,
          Time time,
          val Integer maxProducerIdExpirationMs,
          val Integer producerIdExpirationCheckIntervalMs,
  ...
  public void  initFileSize() : Integer = {
  ...
  @volatile private var LogOffsetMetadata nextOffsetMetadata = _
   ...
   */
  @volatile var Option firstUnstableOffset<LogOffsetMetadata> = None

  /* the actual segments of the log */
  private val ConcurrentNavigableMap segments<java.lang.Long, LogSegment> = new ConcurrentSkipListMap<java.lang.Long, LogSegment>;
  ...
  private public void  initializeLeaderEpochCache(): LeaderEpochCache = {
  ...
  private public void  removeTempFilesAndCollectSwapFiles(): Set<File> = {
  ...
  private public void  loadSegmentFiles(): Unit = {
      ...
      if (isIndexFile(file)) {
        ...
        if (indexFileExists) {
          try {
            ...
            case java e.lang.IllegalArgumentException =>
          ...
          error(String.format("Could not find offset index file corresponding to log file %s, rebuilding index...",segment.log.file.getAbsolutePath))
          recoverSegment(segment);
        }
  ...
  private public void  recoverSegment(LogSegment segment, Option leaderEpochCache<LeaderEpochCache> = None): Integer = lock synchronized {
  ...
  private public void  completeSwapOperations(Set swapFiles<File>): Unit = {
    for (swapFile <- swapFiles) {
      ...
      val swapSegment = new LogSegment(FileRecords.open(swapFile),
        ...
        rollJitterMs = config.randomSegmentJitter,
        time = time);
      info(String.format("Found log file %s from interrupted swap operation, repairing.",swapFile.getPath))
  ...
  private public void  recoverLog() {
    ...
    while(unflushed.hasNext) {
      ...
      info(String.format("Recovering unflushed segment %d in log %s.",segment.baseOffset, name))
      val truncatedBytes =
        try {
          recoverSegment(segment, Some(leaderEpochCache));
        } catch {
          case InvalidOffsetException _ =>
      ...
      if(truncatedBytes > 0) {
        ...
        warn(String.format("Corruption found in segment %d of log %s, truncating to offset %d.",segment.baseOffset, name,
          segment.nextOffset()));
  ...
  private public void  loadProducerState(Long lastOffset, Boolean reloadFromCleanShutdown): Unit = lock synchronized {
  ...
  private public void  loadProducersFromLog(ProducerStateManager producerStateManager, Records records): Unit = {
  ...
  private<log> public void  Map activeProducers<Long, ProducerIdEntry> = lock synchronized {
   ...
   */
  public void  appendAsLeader(MemoryRecords records, Integer leaderEpoch, Boolean isFromClient = true): LogAppendInfo = {
   ...
   * @return Information about the appended messages including the first and last offset.
   */
  public void  appendAsFollower(MemoryRecords records): LogAppendInfo = {
   ...
   * @return Information about the appended messages including the first and last offset.
   */
  private public void  append(MemoryRecords records, Boolean isFromClient, Boolean assignOffsets, Integer leaderEpoch): LogAppendInfo = {
  ...
  public void  onHighWatermarkIncremented(Long highWatermark): Unit = {
    lock synchronized {
  ...
  private public void  updateFirstUnstableOffset(): Unit = lock synchronized {
  ...
  private public void  analyzeAndValidateProducerState(MemoryRecords records, Boolean isFromClient):;
   ...
   * </ol>
   */
  private public void  analyzeAndValidateRecords(MemoryRecords records, Boolean isFromClient): LogAppendInfo = {
    var shallowMessageCount = 0;
    var validBytesCount = 0;
    var firstOffset = -1L;
    var lastOffset = -1L;
    var CompressionCodec sourceCodec = NoCompressionCodec;
    var monotonic = true;
  ...
  private public void  updateProducers(RecordBatch batch,
                              mutable producers.Map<Long, ProducerAppendInfo>,
                              Boolean loadingFromLog): Option<CompletedTxn> = {
   ...
   */
  private public void  trimInvalidBytes(MemoryRecords records, LogAppendInfo info): MemoryRecords = {
  ...
  private<log> public void  readUncommitted(Long startOffset, Integer maxLength, Option maxOffset<Long> = None,
                                   Boolean minOneMessage = false): FetchDataInfo = {
   ...
   */
  public void  read(Long startOffset, Integer maxLength, Option maxOffset<Long> = None, Boolean minOneMessage = false,
    ...
    if (startOffset > next || segmentEntry == null || startOffset < logStartOffset)
      throw new OffsetOutOfRangeException(String.format("Request for offset %d but we only have log segments in the range %d to %d.",startOffset, logStartOffset, next))
  ...
  private<log> public void  collectAbortedTransactions(Long startOffset, Long upperBoundOffset): List<AbortedTxn> = {
    val segmentEntry = segments.floorEntry(startOffset);
    val allAbortedTxns = ListBuffer.empty<AbortedTxn>;
    public void  accumulator(List abortedTxns<AbortedTxn>): Unit = allAbortedTxns ++= abortedTxns;
    collectAbortedTransactions(logStartOffset, upperBoundOffset, segmentEntry, accumulator);
    allAbortedTxns.toList;
  }

  private public void  addAbortedTransactions(Long startOffset, JEntry segmentEntry<JLong, LogSegment>,
    ...
    public void  accumulator(List abortedTxns<AbortedTxn>): Unit = abortedTransactions ++= abortedTxns.map(_.asAbortedTransaction);
  ...
  private public void  collectAbortedTransactions(Long startOffset, Long upperBoundOffset,
                                         JEntry startingSegmentEntry<JLong, LogSegment>,
                                         List accumulator<AbortedTxn> => Unit): Unit = {
   ...
   */
  public void  fetchOffsetsByTimestamp(Long targetTimestamp): Option<TimestampOffset> = {
  ...
  public void  convertToOffsetMetadata(Long offset): LogOffsetMetadata = {
    try {
      ...
      case OffsetOutOfRangeException _ => LogOffsetMetadata.UnknownOffsetMetadata;
    }
  }
   ...
   */
  private public void  deleteOldSegments(LogSegment predicate => Boolean): Integer = {
    lock synchronized {
  ...
  private public void  deleteSegments(Iterable deletable<LogSegment>): Integer = {
  ...
  /**
    ...
    */
  private public void  deletableSegments(LogSegment predicate => Boolean) = {
  ...
  /**
    ...
    */
  public void  deleteOldSegments(): Integer = {
    if (!config.delete) return 0;
    deleteRetentionMsBreachedSegments() + deleteRetentionSizeBreachedSegments() + deleteLogStartOffsetBreachedSegments();
  }

  private public void  deleteRetentionMsBreachedSegments() : Integer = {
    if (config.retentionMs < 0) return 0;
    val startMs = time.milliseconds;
    deleteOldSegments(startMs - _.largestTimestamp > config.retentionMs);
  }

  private public void  deleteRetentionSizeBreachedSegments() : Integer = {
  ...
  private public void  deleteLogStartOffsetBreachedSegments() : Integer = {
   ...
   */
  private public void  maybeRoll Integer messagesSize, Long maxTimestampInMessages, Long maxOffsetInMessages): LogSegment = {
   ...
   */
  public void  roll(Long expectedNextOffset = 0): LogSegment = {
  ...
  public void  minSnapshotOffsetToRetain(Long flushedOffset) = {
  ...
  // visible for testing;
  private<log> public void  takeProducerSnapshot(): Unit = lock synchronized {
    producerStateManager.takeSnapshot();
  }

  // visible for testing;
  private<log> public void  Option latestProducerSnapshotOffset<Long> = lock synchronized {
    producerStateManager.latestSnapshotOffset;
  }

  // visible for testing;
  private<log> public void  Option oldestProducerSnapshotOffset<Long> = lock synchronized {
    producerStateManager.oldestSnapshotOffset;
  }

  // visible for testing;
  private<log> public void  Long latestProducerStateEndOffset = lock synchronized {
   ...
   */
  public void  Iterable logSegments<LogSegment> = segments.values.asScala;
   ...
   */
  private<log> public void  replaceSegments(LogSegment newSegment, Seq oldSegments<LogSegment>, isRecoveredSwapFile : Boolean = false) {
    lock synchronized {
   ...
   */
  private<log> public void  removeLogMetrics(): Unit = {
...
object Log {
  ...
  public void  apply(File dir,
            ...
            Integer maxProducerIdExpirationMs = 60 * 60 * 1000,
            Integer producerIdExpirationCheckIntervalMs = 10 * 60 * 1000): Log = {
   ...
   */
  public void  logFile(File dir, Long offset) =
    new File(dir, filenamePrefixFromOffset(offset) + LogFileSuffix);

  /**
    * Return a directory name to rename the log directory to for async deletion. The name will be in the following
    * topic format-partition.uniqueId-delete where topic, partition and uniqueId are variables.
    */
  public void  logDeleteDirName(String logName): String = {
   ...
   * @param offset The base offset of the log file
   */
  public void  offsetIndexFile(File dir, Long offset) =
   ...
   * @param offset The base offset of the log file
   */
  public void  timeIndexFile(File dir, Long offset) =
   ...
   */
  public void  producerSnapshotFile(File dir, Long offset) =
    new File(dir, filenamePrefixFromOffset(offset) + PidSnapshotFileSuffix);

  public void  transactionIndexFile(File dir, Long offset) =
    new File(dir, filenamePrefixFromOffset(offset) + TxnIndexFileSuffix);

  public void  offsetFromFilename(String filename): Long =
    filename.substring(0, filename.indexOf('.')).toLong;

  /**
   * Parse the topic and partition out of the directory name of a log
   */
  public void  parseTopicPartitionName(File dir): TopicPartition = {
    if (dir == null)
      throw new KafkaException("dir should not be null");

    public void  exception(File dir): KafkaException = {
    ...
    val partition =
      ...
      catch { case NumberFormatException _ => throw exception(dir) }

    new TopicPartition(topic, partition);
  }

  private public void  isIndexFile(File file): Boolean = {
    val filename = file.getName;
    filename.endsWith(IndexFileSuffix) || filename.endsWith(TimeIndexFileSuffix) || filename.endsWith(TxnIndexFileSuffix);
  }

  private public void  isLogFile(File file): Boolean =
