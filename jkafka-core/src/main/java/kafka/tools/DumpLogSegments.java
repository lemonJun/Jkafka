...
object DumpLogSegments {
  ...
  public void  main(Array args<String>) {
    ...
    val filesOpt = parser.accepts("files", "The REQUIRED comma separated list of data and index log files to be dumped.");
                           .withRequiredArg;
    ...
    val messageParser = if (options.has(offsetsOpt)) {
      ...
      val Decoder valueDecoder[_] = CoreUtils.createObject<Decoder[_]>(options.valueOf(valueDecoderOpt), new VerifiableProperties)
      val Decoder keyDecoder[_] = CoreUtils.createObject<Decoder[_]>(options.valueOf(keyDecoderOpt), new VerifiableProperties)
    ...
    misMatchesForIndexFilesMap.foreach {
      case (fileName, listOfMismatches) => {
        ...
        listOfMismatches.foreach(m => {
          System.err.println(String.format("  Index offset: %d, log offset: %d",m._1, m._2))
        });
      }
    }
    ...
    nonConsecutivePairsForLogFilesMap.foreach {
      case (fileName, listOfNonConsecutivePairs) => {
        ...
        listOfNonConsecutivePairs.foreach(m => {
          System.err.println(String.format("  %d is followed by %d",m._1, m._2))
        });
      }
    }
  }

  private public void  dumpTxnIndex(File file): Unit = {
  ...
  private public void  dumpPidSnapshot(File file): Unit = {
    try {
  ...
  private public void  dumpIndex(File file,
                        Boolean indexSanityOnly,
                        Boolean verifyOnly,
                        mutable misMatchesForIndexFilesMap.HashMap[String, List<(Long, Long)]>,
    ...
    for(i <- 0 until index.entries) {
      ...
      if(entry.offset == 0 && i > 0)
        return;
      if (!verifyOnly)
        println(String.format("offset: %d position: %d",entry.offset + index.baseOffset, entry.position))
    }
  }

  private public void  dumpTimeIndex(File file,
                            Boolean indexSanityOnly,
                            Boolean verifyOnly,
    ...
    for(i <- 0 until timeIndex.entries) {
      ...
      if (!verifyOnly)
        println(String.format("timestamp: %s offset: %s",entry.timestamp, timeIndex.baseOffset + entry.offset))
      prevTimestamp = entry.timestamp;
    }
  }

  private trait MessageParser<K, V> {
    public void  parse(Record record): (Option<K], Option[V>);
  }

  private class DecoderMessageParser<K, V>(Decoder keyDecoder[K], Decoder valueDecoder[V]) extends MessageParser<K, V> {
    override public void  parse(Record record): (Option<K], Option[V>) = {
      if (!record.hasValue) {
        (None, None);
      } else {
  ...
  private class TransactionLogMessageParser extends MessageParser<String, String> {
    ...
    override public void  parse(Record record): (Option<String], Option[String>) = {
  ...
  private class OffsetsMessageParser extends MessageParser<String, String> {
    private public void  hex(Array bytes<Byte>): String = {
      if (bytes.isEmpty)
        "";
      else;
    ...
    private public void  parseOffsets(OffsetKey offsetKey, ByteBuffer payload) = {
    ...
    private public void  parseGroupMetadata(GroupMetadataKey groupMetadataKey, ByteBuffer payload) = {
  ...
  private public void  dumpLog(File file,
                      Boolean printContents,
                      mutable nonConsecutivePairsForLogFilesMap.HashMap[String, List<(Long, Long)]>,
                      Boolean isDeepIteration,
                      Integer maxMessageSize,
                      MessageParser parser<_, _>) {
    val startOffset = file.getName.split("\\.")(0).toLong;
    ...
    if(trailingBytes > 0)
      println(String.format("Found %d invalid bytes at the end of %s",trailingBytes, file.getName))
  }
    ...
    public void  printErrors() {
      misMatchesForTimeIndexFilesMap.foreach {
        case (fileName, listOfMismatches) => {
          ...
          listOfMismatches.foreach(m => {
            System.err.println(String.format("  Index timestamp: %d, log timestamp: %d",m._1, m._2))
          });
        }
      }
      ...
      outOfOrderTimestamp.foreach {
        case (fileName, outOfOrderTimestamps) => {
          ...
          outOfOrderTimestamps.foreach(m => {
            System.err.println(String.format("  Index timestamp: %d, Previously indexed timestamp: %d",m._1, m._2))
          });
        }
      }
