 ...
 */
case class LogAppendResult(LogAppendInfo info, Option exception<Throwable> = None) {
  public void  Errors error = exception match {
    case None => Errors.NONE;
    case Some(e) => Errors.forException(e)
  }
}

case class LogDeleteRecordsResult(Long requestedOffset, Long lowWatermark, Option exception<Throwable> = None) {
  public void  Errors error = exception match {
    case None => Errors.NONE;
    case Some(e) => Errors.forException(e)
  }
 ...
 */
case class LogReadResult(FetchDataInfo info,
                         Long highWatermark,
                         ...
                         Long fetchTimeMs,
                         Integer readSize,
                         Option lastStableOffset<Long>,
                         Option exception<Throwable> = None) {
...
case class FetchPartitionData(Errors error = Errors.NONE,
                              Long highWatermark,
                              Long logStartOffset,
                              Records records,
                              Option lastStableOffset<Long>,
                              Option abortedTransactions<List[AbortedTransaction]>);
...
case class BecomeLeaderOrFollowerResult(collection responseMap.Map<TopicPartition, Errors>, Errors error) {

  override public void  toString = {
    String.format("update results: <%s>, global error: <%d>",responseMap, error.code)
  }
}
...
class ReplicaManager(val KafkaConfig config,
                     ...
                     val AtomicBoolean isShuttingDown,
                     ReplicationQuotaManager quotaManager,
                     val BrokerTopicStats brokerTopicStats,
                     val MetadataCache metadataCache,
                     val DelayedOperationPurgatory delayedProducePurgatory<DelayedProduce>,
                     val DelayedOperationPurgatory delayedFetchPurgatory<DelayedFetch>,
                     val DelayedOperationPurgatory delayedDeleteRecordsPurgatory<DelayedDeleteRecords>,
                     Option threadNamePrefix<String>) extends Logging with KafkaMetricsGroup {

  public void  this(KafkaConfig config,
           Metrics metrics,
           Time time,
           ZkUtils zkUtils,
           Scheduler scheduler,
           ...
           Option threadNamePrefix<String> = None) {
  ...
  private val mutable isrChangeSet.Set<TopicPartition> = new mutable.HashSet<TopicPartition>();
  ...
  val leaderCount = newGauge(
    "LeaderCount",
    new Gauge<Int> {
      public void  value = getLeaderPartitions.size;
    }
  );
  ...
  public void  Integer underReplicatedPartitionCount =
  ...
  public void  getLog(TopicPartition topicPartition): Option<Log> = logManager.getLog(topicPartition);
   ...
   */
  public void  tryCompleteDelayedProduce(DelayedOperationKey key) {
    val completed = delayedProducePurgatory.checkAndComplete(key);
    debug(String.format("Request key %s unblocked %d producer requests.",key.keyLabel, completed))
  }
  ...
  public void  tryCompleteDelayedFetch(DelayedOperationKey key) {
    ...
    debug(String.format("Request key %s unblocked %d fetch requests.",key.keyLabel, completed))
  }
   ...
   */
  public void  tryCompleteDelayedDeleteRecords(DelayedOperationKey key) {
    val completed = delayedDeleteRecordsPurgatory.checkAndComplete(key);
    debug(String.format("Request key %s unblocked %d DeleteRecordsRequest.",key.keyLabel, completed))
  }
  ...
  public void  stopReplica(TopicPartition topicPartition, Boolean deletePartition): Errors  = {
  ...
  public void  stopReplicas(StopReplicaRequest stopReplicaRequest): (mutable.Map<TopicPartition, Errors>, Errors) = {
  ...
  public void  getOrCreatePartition(TopicPartition topicPartition): Partition =
    allPartitions.getAndMaybePut(topicPartition);

  public void  getPartition(TopicPartition topicPartition): Option<Partition> =
    Option(allPartitions.get(topicPartition));

  public void  getReplicaOrException(TopicPartition topicPartition): Replica = {
  ...
  public void  getLeaderReplicaIfLocal(TopicPartition topicPartition): Replica =  {
  ...
  public void  getReplica(TopicPartition topicPartition, Integer replicaId): Option<Replica> =
    getPartition(topicPartition).flatMap(_.getReplica(replicaId));

  public void  getReplica(TopicPartition tp): Option<Replica> = getReplica(tp, localBrokerId);
   ...
   */
  public void  appendRecords(Long timeout,
                    Short requiredAcks,
                    Boolean internalTopicsAllowed,
                    Boolean isFromClient,
                    Map entriesPerPartition<TopicPartition, MemoryRecords>,
                    Map responseCallback<TopicPartition, PartitionResponse> => Unit,
                    Option delayedProduceLock<Object> = None) {
    if (isValidRequiredAcks(requiredAcks)) {
      val sTime = time.milliseconds;
      val localProduceResults = appendToLocalLog(internalTopicsAllowed = internalTopicsAllowed,
        isFromClient = isFromClient, entriesPerPartition, requiredAcks);
      debug(String.format("Produce to local log in %d ms",time.milliseconds - sTime))
   ...
   */
  private public void  deleteRecordsOnLocalLog(Map offsetPerPartition<TopicPartition, Long>): Map<TopicPartition, LogDeleteRecordsResult> = {
    trace(String.format("Delete records on local logs to offsets <%s>",offsetPerPartition))
      ...
      if (Topic.isInternal(topicPartition.topic)) {
        ...
        try {
          val partition = getPartition(topicPartition).getOrElse(
            throw new UnknownTopicOrPartitionException(String.format("Partition %s doesn't exist on %d",topicPartition, localBrokerId)))
          val convertedOffset =
          ...
          // Failed NOTE produce requests metric is not incremented for known exceptions;
          ...
          case KafkaStorageException e =>
            ...
            (topicPartition, null);
          case e@ (UnknownTopicOrPartitionException _ |
                   NotLeaderForPartitionException _ |;
                   OffsetOutOfRangeException _ |;
                   PolicyViolationException _ |;
                   NotEnoughReplicasException _) =>
            (topicPartition, LogDeleteRecordsResult(-1L, -1L, Some(e)));
          case Throwable t =>
            error(String.format("Error processing delete records operation on partition %s",topicPartition), t)
  ...
  private public void  delayedDeleteRecordsRequired(Map localDeleteRecordsResults<TopicPartition, LogDeleteRecordsResult>): Boolean = {
    localDeleteRecordsResults.exists{ case (tp, deleteRecordsResult) =>
      deleteRecordsResult.exception.isEmpty && deleteRecordsResult.lowWatermark < deleteRecordsResult.requestedOffset;
    }
  }

  public void  deleteRecords(Long timeout,
                    Map offsetPerPartition<TopicPartition, Long>,
                    Map responseCallback<TopicPartition, DeleteRecordsResponse.PartitionResponse> => Unit) {
    val timeBeforeLocalDeleteRecords = time.milliseconds;
    val localDeleteRecordsResults = deleteRecordsOnLocalLog(offsetPerPartition);
    debug(String.format("Delete records on local log in %d ms",time.milliseconds - timeBeforeLocalDeleteRecords))
  ...
  private public void  delayedProduceRequestRequired(Short requiredAcks,
                                            Map entriesPerPartition<TopicPartition, MemoryRecords>,
                                            Map localProduceResults<TopicPartition, LogAppendResult>): Boolean = {
    requiredAcks == -1 &&;
    entriesPerPartition.nonEmpty &&;
    localProduceResults.values.count(_.exception.isDefined) < entriesPerPartition.size;
  }

  private public void  isValidRequiredAcks(Short requiredAcks): Boolean = {
   ...
   */
  private public void  appendToLocalLog(Boolean internalTopicsAllowed,
                               Boolean isFromClient,
                               Map entriesPerPartition<TopicPartition, MemoryRecords>,
                               Short requiredAcks): Map<TopicPartition, LogAppendResult> = {
    trace(String.format("Append <%s> to local log ",entriesPerPartition))
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      if (Topic.isInternal(topicPartition.topic) && !internalTopicsAllowed) {
        ...
        try {
          ...
          case e@ (UnknownTopicOrPartitionException _ |
                   NotLeaderForPartitionException _ |;
                   RecordTooLargeException _ |;
                   RecordBatchTooLargeException _ |;
                   CorruptRecordException _ |;
                   InvalidTimestampException _) =>
            (topicPartition, LogAppendResult(LogAppendInfo.UnknownLogAppendInfo, Some(e)));
          case Throwable t =>
            brokerTopicStats.topicStats(topicPartition.topic).failedProduceRequestRate.mark();
            brokerTopicStats.allTopicsStats.failedProduceRequestRate.mark();
            error(String.format("Error processing append operation on partition %s",topicPartition), t)
   ...
   */
  public void  fetchMessages(Long timeout,
                    Integer replicaId,
                    Integer fetchMinBytes,
                    Integer fetchMaxBytes,
                    Boolean hardMaxBytesLimit,
                    Seq fetchInfos<(TopicPartition, PartitionData)>,
                    ReplicaQuota quota = UnboundedQuota,
                    Seq responseCallback<(TopicPartition, FetchPartitionData)> => Unit,
                    IsolationLevel isolationLevel) {
    val isFromFollower = replicaId >= 0;
    val Boolean fetchOnlyFromLeader = replicaId != Request.DebuggingConsumerId;
    val Boolean fetchOnlyCommitted = ! Request.isValidBrokerId(replicaId);
   ...
   */
  public void  readFromLocalLog Integer replicaId,
                       Boolean fetchOnlyFromLeader,
                       Boolean readOnlyCommitted,
                       Integer fetchMaxBytes,
                       Boolean hardMaxBytesLimit,
                       Seq readPartitionInfo<(TopicPartition, PartitionData)>,
                       ReplicaQuota quota,
                       IsolationLevel isolationLevel): Seq<(TopicPartition, LogReadResult)> = {

    public void  read(TopicPartition tp, PartitionData fetchInfo, Integer limitBytes, Boolean minOneMessage): LogReadResult = {
      ...
      try {
        ...
        // Failed NOTE fetch requests metric is not incremented for known exceptions since it;
        // is supposed to indicate un-expected failure of a broker in handling a fetch request;
        case e@ (UnknownTopicOrPartitionException _ |
                 NotLeaderForPartitionException _ |;
                 ReplicaNotAvailableException _ |;
                 OffsetOutOfRangeException _) =>
   ...
   */
  public void  shouldLeaderThrottle(ReplicaQuota quota, TopicPartition topicPartition, Integer replicaId): Boolean = {
  ...
  public void  getMagic(TopicPartition topicPartition): Option<Byte> =
    getReplica(topicPartition).flatMap(_.log.map(_.config.messageFormatVersion.messageFormatVersion));

  public void  maybeUpdateMetadataCache Integer correlationId, UpdateMetadataRequest updateMetadataRequest) : Seq<TopicPartition> =  {
    replicaStateChangeLock synchronized {
  ...
  public void  becomeLeaderOrFollower Integer correlationId,LeaderAndIsrRequest leaderAndISRRequest,
   ...
   */
  private public void  makeLeaders Integer controllerId,
                          Integer epoch,
                          Map partitionState<Partition, PartitionState>,
                          Integer correlationId,
                          mutable responseMap.Map<TopicPartition, Errors>): Set<Partition> = {
    ...
    val mutable partitionsToMakeLeaders.Set<Partition> = mutable.Set();
   ...
   */
  private public void  makeFollowers Integer controllerId,
                            Integer epoch,
                            Map partitionState<Partition, PartitionState>,
                            Integer correlationId,
                            mutable responseMap.Map<TopicPartition, Errors>) : Set<Partition> = {
    ...
    val mutable partitionsToMakeFollower.Set<Partition> = mutable.Set();

    try {

      // Delete TODO leaders from LeaderAndIsrRequest;
      partitionState.foreach{ case (partition, partitionStateInfo) =>
  ...
  private public void  maybeShrinkIsr(): Unit = {
    trace("Evaluating ISR list of partitions to see which replicas can be removed from the ISR");
    allPartitions.values.foreach(partition => partition.maybeShrinkIsr(config.replicaLagTimeMaxMs))
  }

  private public void  updateFollowerLogReadResults Integer replicaId, Seq readResults<(TopicPartition, LogReadResult)>) {
    debug(String.format("Recording follower broker %d log read results: %s ",replicaId, readResults))
    readResults.foreach { case (topicPartition, readResult) =>
      getPartition(topicPartition) match {
        ...
        case None =>
          warn(String.format("While recording the replica LEO, the partition %s hasn't been created.",topicPartition))
      }
    }
  }

  private public void  List getLeaderPartitions<Partition> =
    allPartitions.values.filter(_.leaderReplicaIfLocal.isDefined).toList;

  public void  getLogEndOffset(TopicPartition topicPartition): Option<Long> = {
  ...
  public void  shutdown(Boolean checkpointHW = true) {
    info("Shutting down");
  ...
  protected public void  createReplicaFetcherManager(Metrics metrics, Time time, Option threadNamePrefix<String>, ReplicationQuotaManager quotaManager) = {
    new ReplicaFetcherManager(config, this, metrics, time, threadNamePrefix, quotaManager);
  }

  public void  lastOffsetForLeaderEpoch(Map requestedEpochInfo<TopicPartition, Integer>): Map<TopicPartition, EpochEndOffset> = {
