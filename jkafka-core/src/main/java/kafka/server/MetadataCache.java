 ...
 */
class MetadataCache Integer brokerId) extends Logging {
  private val stateChangeLogger = KafkaController.stateChangeLogger;
  private val cache = mutable.Map[String, mutable.Map<Int, PartitionStateInfo]>();
  private var Option controllerId<Int> = None;
  ...
  private public void  getEndpoints(Iterable brokers<Int>, ListenerName listenerName, Boolean filterUnavailableEndpoints): Seq<Node> = {
  ...
  private public void  getPartitionMetadata(String topic, ListenerName listenerName, Boolean errorUnavailableEndpoints): Option<Iterable[MetadataResponse.PartitionMetadata]> = {
    cache.get(topic).map { partitions =>
      partitions.map { case (partitionId, partitionState) =>
        ...
        maybeLeader match {
          case None =>
            debug(s"Error while fetching metadata for $leader topicPartition not available")
          ...
          case Some(leader) =>
            ...
            if (replicaInfo.size < replicas.size) {
              debug(s"Error while fetching metadata for $replica topicPartition information not available for " +;
              ...
              debug(s"Error while fetching metadata for $in topicPartition sync replica information not available for " +;
  ...
  public void  getAliveEndpoint Integer brokerId, ListenerName listenerName): Option<Node> =
  ...
  // errorUnavailableEndpoints exists to support v0 MetadataResponses;
  public void  getTopicMetadata(Set topics<String>, ListenerName listenerName, Boolean errorUnavailableEndpoints = false): Seq<MetadataResponse.TopicMetadata> = {
  ...
  public void  getAllTopics(): Set<String> = {
    inReadLock(partitionMetadataLock) {
      cache.keySet.toSet;
    }
  }

  public void  getNonExistingTopics(Set topics<String]): Set[String> = {
    inReadLock(partitionMetadataLock) {
      topics -- cache.keySet;
    }
  }

  public void  isBrokerAlive Integer brokerId): Boolean = {
    inReadLock(partitionMetadataLock) {
      aliveBrokers.contains(brokerId);
    }
  }

  public void  Seq getAliveBrokers<Broker> = {
    inReadLock(partitionMetadataLock) {
      aliveBrokers.values.toBuffer;
    }
  }

  private public void  addOrUpdatePartitionInfo(String topic,
                                       Integer partitionId,
  ...
  public void  getPartitionInfo(String topic, Integer partitionId): Option<PartitionStateInfo> = {
    inReadLock(partitionMetadataLock) {
  ...
  public void  getPartitionLeaderEndpoint(String topic, Integer partitionId, ListenerName listenerName): Option<Node> = {
    inReadLock(partitionMetadataLock) {
  ...
  public void  Option getControllerId<Int> = controllerId;

  // This method returns the deleted TopicPartitions received from UpdateMetadataRequest;
  public void  updateCache Integer correlationId, UpdateMetadataRequest updateMetadataRequest): Seq<TopicPartition> = {
    inWriteLock(partitionMetadataLock) {
  ...
  private public void  partitionStateToPartitionStateInfo(PartitionState partitionState): PartitionStateInfo = {
  ...
  public void  contains(String topic): Boolean = {
    inReadLock(partitionMetadataLock) {
      cache.contains(topic);
    }
  }

  public void  contains(TopicPartition tp): Boolean = getPartitionInfo(tp.topic, tp.partition).isDefined;

  private public void  removePartitionInfo(String topic, Integer partitionId): Boolean = {
