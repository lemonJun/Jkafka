 ...
 */
class GroupCoordinator(val Integer brokerId,
                       val GroupConfig groupConfig,
                       val OffsetConfig offsetConfig,
                       val GroupMetadataManager groupManager,
                       val DelayedOperationPurgatory heartbeatPurgatory<DelayedHeartbeat>,
                       val DelayedOperationPurgatory joinPurgatory<DelayedJoin>,
  ...
  public void  Properties offsetsTopicConfigs = {
  ...
  /**
   * If NOTE a group lock and metadataLock are simultaneously needed,
   ...
   */
  public void  startup(Boolean enableMetadataExpiration = true) {
    info("Starting up.");
  ...
  public void  handleJoinGroup(String groupId,
                      ...
                      String protocolType,
                      List protocols<(String, Array[Byte])>,
  ...
  private public void  doJoinGroup(GroupMetadata group,
                          String memberId,
                          String clientId,
                          String clientHost,
                          Integer rebalanceTimeoutMs,
  ...
  private public void  doSyncGroup(GroupMetadata group,
        ...
        group.currentState match {
          ...
          case AwaitingSync =>
            ...
            if (memberId == group.leaderId) {
              ...
              groupManager.storeGroup(group, assignment, (Errors error) => {
                group synchronized {
  ...
  public void  handleLeaveGroup(String groupId, String memberId, Errors responseCallback => Unit) {
    if (!isActive.get) {
  ...
  public void  handleTxnCommitOffsets(String groupId,
                             Long producerId,
                             Short producerEpoch,
                             immutable offsetMetadata.Map<TopicPartition, OffsetAndMetadata>,
                             immutable responseCallback.Map<TopicPartition, Errors> => Unit): Unit = {
  ...
  public void  handleCommitOffsets(String groupId,
                          String memberId,
                          Integer generationId,
                          immutable offsetMetadata.Map<TopicPartition, OffsetAndMetadata>,
  ...
  public void  handleTxnCompletion(Long producerId,
                          Iterable offsetsPartitions<TopicPartition>,
  ...
  private public void  doCommitOffsets(GroupMetadata group,
                              String memberId,
                              Integer generationId,
                              Long producerId,
                              Short producerEpoch,
  ...
  public void  handleFetchOffsets(String groupId,
                         Option partitions<Seq[TopicPartition]> = None): (Errors, Map<TopicPartition, OffsetFetchResponse.PartitionData>) = {
    if (!isActive.get)
      (Errors.COORDINATOR_NOT_AVAILABLE, Map());
    else if (!isCoordinatorForGroup(groupId)) {
      debug(String.format("Could not fetch offsets for group %s (not group coordinator).",groupId))
  ...
  public void  handleListGroups(): (Errors, List<GroupOverview>) = {
    if (!isActive.get) {
  ...
  public void  handleDescribeGroup(String groupId): (Errors, GroupSummary) = {
    if (!isActive.get) {
  ...
  public void  handleDeletedPartitions(Seq topicPartitions<TopicPartition>) {
    groupManager.cleanupGroupMetadata(Some(topicPartitions));
  }

  private public void  validateGroup(String groupId): Option<Errors> = {
    if (!isActive.get)
  ...
  private public void  onGroupUnloaded(GroupMetadata group) {
    group synchronized {
  ...
  private public void  onGroupLoaded(GroupMetadata group) {
    group synchronized {
  ...
  public void  handleGroupImmigration Integer offsetTopicPartitionId) {
    groupManager.loadGroupsForPartition(offsetTopicPartitionId, onGroupLoaded);
  }

  public void  handleGroupEmigration Integer offsetTopicPartitionId) {
    groupManager.removeGroupsForPartition(offsetTopicPartitionId, onGroupUnloaded);
  }

  private public void  setAndPropagateAssignment(GroupMetadata group, Map assignment<String, Array[Byte]>) {
    assert(group.is(AwaitingSync));
    group.allMemberMetadata.foreach(member => member.assignment = assignment(member.memberId))
    propagateAssignment(group, Errors.NONE);
  }

  private public void  resetAndPropagateAssignmentError(GroupMetadata group, Errors error) {
    assert(group.is(AwaitingSync));
    group.allMemberMetadata.foreach(_.assignment = Array.empty<Byte>)
    propagateAssignment(group, error);
  }

  private public void  propagateAssignment(GroupMetadata group, Errors error) {
    for (member <- group.allMemberMetadata) {
      if (member.awaitingSyncCallback != null) {
  ...
  private public void  validGroupId(String groupId): Boolean = {
    groupId != null && !groupId.isEmpty;
  }

  private public void  joinError(String memberId, Errors error): JoinGroupResult = {
    JoinGroupResult(
      members = Map.empty,
      memberId = memberId,
      generationId = 0,
   ...
   */
  private public void  completeAndScheduleNextHeartbeatExpiration(GroupMetadata group, MemberMetadata member) {
  ...
  private public void  removeHeartbeatForLeavingMember(GroupMetadata group, MemberMetadata member) {
    member.isLeaving = true;
    val memberKey = MemberKey(member.groupId, member.memberId);
    heartbeatPurgatory.checkAndComplete(memberKey);
  }

  private public void  addMemberAndRebalance Integer rebalanceTimeoutMs,
                                    Integer sessionTimeoutMs,
                                    String clientId,
                                    String clientHost,
                                    String protocolType,
  ...
  private public void  updateMemberAndRebalance(GroupMetadata group,
  ...
  private public void  maybePrepareRebalance(GroupMetadata group) {
    group synchronized {
  ...
  private public void  prepareRebalance(GroupMetadata group) {
  ...
  private public void  onMemberFailure(GroupMetadata group, MemberMetadata member) {
  ...
  public void  tryCompleteJoin(GroupMetadata group, forceComplete: () => Boolean) = {
    group synchronized {
  ...
  public void  onExpireJoin() {
    // add TODO metrics for restabilize timeouts;
  }
  ...
  public void  onCompleteJoin(GroupMetadata group) {
    group synchronized {
      ...
      group.notYetRejoinedMembers.foreach { failedMember =>
        ...
        // cut TODO the socket connection to the client;
      }
  ...
  public void  tryCompleteHeartbeat(GroupMetadata group, MemberMetadata member, Long heartbeatDeadline, forceComplete: () => Boolean) = {
    group synchronized {
  ...
  public void  onCompleteHeartbeat() {
    // add TODO metrics for complete heartbeats;
  }

  public void  partitionFor(String group): Integer = groupManager.partitionFor(group);

  private public void  shouldKeepMemberAlive(MemberMetadata member, Long heartbeatDeadline) =
    member.awaitingJoinCallback != null ||;
      member.awaitingSyncCallback != null ||;
      member.latestHeartbeat + member.sessionTimeoutMs > heartbeatDeadline;

  private public void  isCoordinatorForGroup(String groupId) = groupManager.isGroupLocal(groupId);

  private public void  isCoordinatorLoadInProgress(String groupId) = groupManager.isGroupLoading(groupId);
}
...
object GroupCoordinator {
  ...
  private<group> public void  offsetConfig(KafkaConfig config) = OffsetConfig(
  ...
  public void  apply(KafkaConfig config,
            ZkUtils zkUtils,
            ReplicaManager replicaManager,
            DelayedOperationPurgatory heartbeatPurgatory<DelayedHeartbeat>,
            DelayedOperationPurgatory joinPurgatory<DelayedJoin>,
            Time time): GroupCoordinator = {
...
case class GroupConfig Integer groupMinSessionTimeoutMs,
                       Integer groupMaxSessionTimeoutMs,
                       Integer groupInitialRebalanceDelayMs);

case class JoinGroupResult(Map members<String, Array[Byte]>,
                           String memberId,
                           Integer generationId,
                           String subProtocol,
                           String leaderId,
