...
object TransactionCoordinator {
  ...
  private public void  initTransactionError(Errors error): InitProducerIdResult = {
    InitProducerIdResult(RecordBatch.NO_PRODUCER_ID, RecordBatch.NO_PRODUCER_EPOCH, error);
  }

  private public void  initTransactionMetadata(TxnTransitMetadata txnMetadata): InitProducerIdResult = {
 ...
 */
class TransactionCoordinator Integer brokerId,
                             Scheduler scheduler,
                             ProducerIdManager producerIdManager,
                             TransactionStateManager txnManager,
                             TransactionMarkerChannelManager txnMarkerChannelManager,
  ...
  public void  handleInitProducerId(String transactionalId,
                           ...
                           InitProducerIdCallback responseCallback): Unit = {
    ...
    if (transactionalId == null) {
      ...
      val ApiResult result<(Int, TxnTransitMetadata)> = coordinatorEpochAndMetadata.right.flatMap {
      ...
      result match {
        ...
        case Right((coordinatorEpoch, newMetadata)) =>
          if (newMetadata.txnState == Ongoing) {
            ...
            public void  sendRetriableErrorCallback(Errors error): Unit = {
            ...
            public void  sendPidResponseCallback(Errors error): Unit = {
              if (error == Errors.NONE) {
  ...
  private public void  prepareInitProduceIdTransit(String transactionalId,
                                          Integer transactionTimeoutMs,
                                          Integer coordinatorEpoch,
                                          TransactionMetadata txnMetadata): ApiResult<(Int, TxnTransitMetadata)> = {
  ...
  public void  handleAddPartitionsToTransaction(String transactionalId,
                                       Long producerId,
                                       Short producerEpoch,
                                       collection partitions.Set<TopicPartition>,
                                       AddPartitionsCallback responseCallback): Unit = {
    if (transactionalId == null || transactionalId.isEmpty) {
      ...
      val ApiResult result<(Int, TxnTransitMetadata)> = txnManager.getTransactionState(transactionalId).right.flatMap {
        ...
        case Some(epochAndMetadata) =>
          ...
          txnMetadata synchronized {
            if (txnMetadata.producerId != producerId) {
              ...
              // this is an if optimization the partitions are already in the metadata reply OK immediately;
              Left(Errors.NONE);
            } else {
  ...
  public void  handleTxnImmigration Integer txnTopicPartitionId, Integer coordinatorEpoch) {
  ...
  public void  handleTxnEmigration Integer txnTopicPartitionId, Integer coordinatorEpoch) {
  ...
  private public void  logInvalidStateTransitionAndReturnError(String transactionalId,
  ...
  public void  handleEndTransaction(String transactionalId,
                           ...
                           EndTxnCallback responseCallback): Unit = {
    if (transactionalId == null || transactionalId.isEmpty)
      responseCallback(Errors.INVALID_REQUEST);
    else {
      val ApiResult preAppendResult<(Int, TxnTransitMetadata)> = txnManager.getTransactionState(transactionalId).right.flatMap {
        case None =>
          Left(Errors.INVALID_PRODUCER_ID_MAPPING);
      ...
      preAppendResult match {
        ...
        case Right((coordinatorEpoch, newMetadata)) =>
          public void  sendTxnMarkersCallback(Errors error): Unit = {
            if (error == Errors.NONE) {
              val ApiResult preSendResult<(TransactionMetadata, TxnTransitMetadata)> = txnManager.getTransactionState(transactionalId).right.flatMap {
                case None =>
  ...
  public void  Properties transactionTopicConfigs = txnManager.transactionTopicConfigs;

  public void  partitionFor(String transactionalId): Integer = txnManager.partitionFor(transactionalId);

  private public void  abortTimedOutTransactions(): Unit = {
   ...
   */
  public void  startup(Boolean enableTransactionalIdExpiration = true) {
    info("Starting up.");
    scheduler.startup();
...
case class InitProducerIdResult(Long producerId, Short producerEpoch, Errors error);
