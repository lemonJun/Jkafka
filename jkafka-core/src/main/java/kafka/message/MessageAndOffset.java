...
object MessageAndOffset {
  public void  fromRecordBatch(RecordBatch batch): MessageAndOffset = {
    batch match {
      case AbstractLegacyRecordBatch legacyBatch =>
  ...
  public void  fromRecord(Record record): MessageAndOffset = {
    record match {
      case AbstractLegacyRecordBatch legacyBatch =>
        MessageAndOffset(Message.fromRecord(legacyBatch.outerRecord), legacyBatch.lastOffset);
...
case class MessageAndOffset(Message message, Long offset) {
  ;
  /**
   * Compute the offset of the next message in the log
   */
  public void  Long nextOffset = offset + 1;
