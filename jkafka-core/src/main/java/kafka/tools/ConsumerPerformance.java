...
object ConsumerPerformance {
  ...
  public void  main(Array args<String>): Unit = {
    ...
    var mutable metrics.Map[MetricName, _ <: Metric] = null;
    ...
    if (!config.showDetailedStats) {
      ...
      println(String.format("%s, %s, %.4f, %.4f, %d, %.4f",config.dateFormat.format(startMs), config.dateFormat.format(endMs),
  ...
  public void  consume(KafkaConsumer consumer<Array[Byte], Array[Byte]>, List topics<String>, Long count, Long timeout, ConsumerPerfConfig config, AtomicLong totalMessagesRead, AtomicLong totalBytesRead) {
    var bytesRead = 0L;
    var messagesRead = 0L;
    ...
    consumer.subscribe(topics.asJava, new ConsumerRebalanceListener {
      public void  onPartitionsAssigned(util partitions.Collection<TopicPartition>) {
        isAssigned.set(true);
      }
      public void  onPartitionsRevoked(util partitions.Collection<TopicPartition>) {
    ...
    var Long lastReportTime = startMs;
  ...
  public void  printProgressMessage Integer id, Long bytesRead, Long lastBytesRead, Long messagesRead, Long lastMessagesRead,
    Long startMs, Long endMs, SimpleDateFormat dateFormat) = {
    val Double elapsedMs = endMs - startMs;
    val totalMBRead = (bytesRead * 1.0) / (1024 * 1024);
    val mbRead = ((bytesRead - lastBytesRead) * 1.0) / (1024 * 1024);
    println(String.format("%s, %d, %.4f, %.4f, %d, %.4f",dateFormat.format(endMs), id, totalMBRead,
        1000.0 * (mbRead / elapsedMs), messagesRead, ((messagesRead - lastMessagesRead) / elapsedMs) * 1000.0));
  }

  class ConsumerPerfConfig(Array args<String>) extends PerfConfig(args) {
    ...
    val bootstrapServersOpt = parser.accepts("broker-list", "REQUIRED (unless old consumer is used): A broker list to use for connecting if using the new consumer.")
      ...
      .describedAs("host");
      .ofType(classOf<String>);
    val topicOpt = parser.accepts("topic", "The REQUIRED topic to consume from.");
      .withRequiredArg;
      .describedAs("topic");
      .ofType(classOf<String>);
  ...
  class ConsumerPerfThread Integer threadId, String name, KafkaStream stream<Array[Byte], Array[Byte]>,
    ConsumerPerfConfig config, AtomicLong totalMessagesRead, AtomicLong totalBytesRead, AtomicBoolean consumerTimeout);
    extends Thread(name) {
    ...
    override public void  run() {
      ...
      try {
        ...
        case ClosedByInterruptException _ =>
        case ConsumerTimeoutException _ =>
          consumerTimeout.set(true);
        case Throwable e => e.printStackTrace();
      }
