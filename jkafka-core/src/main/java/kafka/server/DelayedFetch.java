...
case class FetchPartitionStatus(LogOffsetMetadata startOffsetMetadata, PartitionData fetchInfo) {

  override public void  toString = "[startOffsetMetadata: " + startOffsetMetadata + ", " +;
                          "fetchInfo: " + fetchInfo + "]";
}
 ...
 */
case class FetchMetadata Integer fetchMinBytes,
                         Integer fetchMaxBytes,
                         Boolean hardMaxBytesLimit,
                         Boolean fetchOnlyLeader,
                         Boolean fetchOnlyCommitted,
                         Boolean isFromFollower,
                         Integer replicaId,
                         Seq fetchPartitionStatus<(TopicPartition, FetchPartitionStatus)>) {

  override public void  toString = "[minBytes: " + fetchMinBytes + ", " +;
 ...
 */
class DelayedFetch(Long delayMs,
   ...
   * Case This B broker does not know of some partitions it tries to fetch
   ...
   */
  override public void  tryComplete() : Boolean = {
    ...
    fetchMetadata.fetchPartitionStatus.foreach {
      case (topicPartition, fetchStatus) =>
        ...
        try {
          if (fetchOffset != LogOffsetMetadata.UnknownOffsetMetadata) {
            ...
            if (endOffset.messageOffset != fetchOffset.messageOffset) {
              if (endOffset.onOlderSegment(fetchOffset)) {
                ...
                debug(String.format("Satisfying fetch %s since it is fetching later segments of partition %s.",fetchMetadata, topicPartition))
                return forceComplete()
              } else if (fetchOffset.onOlderSegment(endOffset)) {
                // Case C, this can happen when the fetch operation is falling behind the current segment;
                // or the partition has just rolled a new segment;
                debug(String.format("Satisfying fetch %s immediately since it is fetching older segments.",fetchMetadata))
          ...
          case UnknownTopicOrPartitionException _ => // Case B;
            debug(String.format("Broker no longer know of %s, satisfy %s immediately",topicPartition, fetchMetadata))
            return forceComplete()
          case NotLeaderForPartitionException _ =>  // Case A;
            debug(String.format("Broker is no longer the leader of %s, satisfy %s immediately",topicPartition, fetchMetadata))
            return forceComplete()
        }
    }
  ...
  override public void  onExpiration() {
