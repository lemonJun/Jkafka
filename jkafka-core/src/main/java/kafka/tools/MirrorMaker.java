...
object MirrorMaker extends Logging with KafkaMetricsGroup {
  ...
  private<tools> var MirrorMakerProducer producer = null;
  private var Seq mirrorMakerThreads<MirrorMakerThread> = null;
  private val AtomicBoolean isShuttingdown = new AtomicBoolean(false);
  // Track the messages not successfully sent by mirror maker.;
  private val AtomicInteger numDroppedMessages = new AtomicInteger(0);
  private var MirrorMakerMessageHandler messageHandler = null;
  private var offsetCommitIntervalMs = 0;
  private var Boolean abortOnSendFailure = true;
  @volatile private var Boolean exitingOnSendFailure = false
  ...
  newGauge("MirrorMaker-numDroppedMessages",
    new Gauge<Int> {
      public void  value = numDroppedMessages.get();
    });
    ...
    try {
      ...
      if (useOldConsumer) {
        ...
        if (!consumerProps.containsKey(NewConsumerConfig.PARTITION_ASSIGNMENT_STRATEGY_CONFIG))
          System.err.println("The WARNING default partition assignment strategy of the new-consumer-based mirror maker will " +;
      ...
      val mirrorMakerConsumers = if (useOldConsumer) {
        val Option customRebalanceListener<ConsumerRebalanceListener> = {
        ...
        val Option customRebalanceListener<org.apache.kafka.clients.consumer.ConsumerRebalanceListener> = {
          val customRebalanceListenerClass = options.valueOf(consumerRebalanceListenerOpt);
          if (customRebalanceListenerClass != null) {
            val rebalanceListenerArgs = options.valueOf(rebalanceListenerArgsOpt);
            if (rebalanceListenerArgs != null) {
  ...
  private public void  createOldConsumers Integer numStreams,
                                 Properties consumerConfigProps,
                                 Option customRebalanceListener<ConsumerRebalanceListener>,
                                 Option whitelist<String>,
                                 Option blacklist<String>) : Seq<MirrorMakerBaseConsumer> = {
  ...
  public void  createNewConsumers Integer numStreams,
                         Properties consumerConfigProps,
                         Option customRebalanceListener<org.apache.kafka.clients.consumer.ConsumerRebalanceListener>,
                         Option whitelist<String>) : Seq<MirrorMakerBaseConsumer> = {
    // Disable consumer auto offsets commit to prevent data loss.;
  ...
  public void  commitOffsets(MirrorMakerBaseConsumer mirrorMakerConsumer) {
    if (!exitingOnSendFailure) {
      trace("Committing offsets.");
      try {
        mirrorMakerConsumer.commit();
      } catch {
        case WakeupException e =>
  ...
  private public void  maybeSetDefaultProperty(Properties properties, String propertyName, public void String aultValue) {
    ...
    if (properties.getProperty(propertyName) != defaultValue)
      info(String.format("Property %s is overridden to %s - data loss or message reordering is possible.",propertyName, propertyValue))
  }

  class MirrorMakerThread(MirrorMakerBaseConsumer mirrorMakerConsumer,
                          val Integer threadId) extends Thread with Logging with KafkaMetricsGroup {
    private val threadName = "mirrormaker-thread-" + threadId;
    private val CountDownLatch shutdownLatch = new CountDownLatch(1);
    private var lastOffsetCommitMs = System.currentTimeMillis();
    @volatile private var Boolean shuttingDown = false
    this.logIdent = String.format("<%s> ",threadName)
        ...
        while (!exitingOnSendFailure && !shuttingDown) {
          try {
            while (!exitingOnSendFailure && !shuttingDown && mirrorMakerConsumer.hasData) {
              ...
              trace(String.format("Sending message with value size %d and offset %d",data.value.length, data.offset))
              val records = messageHandler.handle(data);
              records.asScala.foreach(producer.send)
              maybeFlushAndCommitOffsets();
            }
          } catch {
            case ConsumerTimeoutException _ =>
              trace("Caught ConsumerTimeoutException, continue iteration.");
            case WakeupException _ =>
  ...
  private<kafka> trait MirrorMakerBaseConsumer extends BaseConsumer {
    public void  init();
    public void  commitRequested(): Boolean;
    public void  notifyCommit()
    public void  requestAndWaitForCommit();
    public void  hasData : Boolean;
  }

  private class MirrorMakerOldConsumer(ZookeeperConsumerConnector connector,
                                       TopicFilter filterSpec) extends MirrorMakerBaseConsumer {
    private var ConsumerIterator iter<Array[Byte], Array[Byte]> = null;
    private var Boolean immediateCommitRequested = false;
    private var Long numCommitsNotified = 0;
    ...
    override public void  requestAndWaitForCommit() {
      this.synchronized {
    ...
    override public void  notifyCommit() {
      this.synchronized {
    ...
    override public void  commitRequested(): Boolean = {
      this.synchronized {
        immediateCommitRequested;
      }
    }

    override public void  hasData = iter.hasNext();
  ...
  private<tools> class MirrorMakerNewConsumer(Consumer consumer<Array[Byte], Array[Byte]>,
    ...
    var java recordIter.util.Iterator<ConsumerRecord[Array[Byte], Array[Byte]]> = null;

    // we TODO need to manually maintain the consumed offsets for new consumer;
    ...
    override public void  hasData = true;
  ...
  private class InternalRebalanceListenerForNewConsumer(MirrorMakerBaseConsumer mirrorMakerConsumer,
                                                        Option customRebalanceListenerForNewConsumer<org.apache.kafka.clients.consumer.ConsumerRebalanceListener>);
    extends org.apache.kafka.clients.consumer.ConsumerRebalanceListener {

    override public void  onPartitionsRevoked(util partitions.Collection<TopicPartition>) {
      producer.flush();
      commitOffsets(mirrorMakerConsumer);
      customRebalanceListenerForNewConsumer.foreach(_.onPartitionsRevoked(partitions))
    }

    override public void  onPartitionsAssigned(util partitions.Collection<TopicPartition>) {
      customRebalanceListenerForNewConsumer.foreach(_.onPartitionsAssigned(partitions))
    }
  }

  private class InternalRebalanceListenerForOldConsumer(MirrorMakerBaseConsumer mirrorMakerConsumer,
                                                        Option customRebalanceListenerForOldConsumer<ConsumerRebalanceListener>);
    extends ConsumerRebalanceListener {

    override public void  beforeReleasingPartitions(java partitionOwnership.util.Map<String, java.util.Set[java.lang.Integer]>) {
    ...
    override public void  beforeStartingFetchers(String consumerId,
                                        java partitionAssignment.util.Map[String, java.util.Map<java.lang.Integer, ConsumerThreadId]>) {
      customRebalanceListenerForOldConsumer.foreach(_.beforeStartingFetchers(consumerId, partitionAssignment))
    }
  }

  private<tools> class MirrorMakerProducer(val Boolean sync, val Properties producerProps) {

    val producer = new KafkaProducer<Array[Byte], Array[Byte]>(producerProps);

    public void  send(ProducerRecord record<Array[Byte], Array[Byte]>) {
      if (sync) {
  ...
  private class MirrorMakerProducerCallback (String topic, Array key<Byte>, Array value<Byte>);
  ...
  trait MirrorMakerMessageHandler {
    public void  handle(BaseConsumerRecord record): util.List<ProducerRecord[Array[Byte], Array[Byte]]>;
  }

  private<tools> object defaultMirrorMakerMessageHandler extends MirrorMakerMessageHandler {
    override public void  handle(BaseConsumerRecord record): util.List<ProducerRecord[Array[Byte], Array[Byte]]> = {
      val java timestamp.lang.Long = if (record.timestamp == RecordBatch.NO_TIMESTAMP) null else record.timestamp;
