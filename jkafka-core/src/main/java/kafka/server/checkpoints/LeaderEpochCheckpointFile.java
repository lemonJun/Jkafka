...
object LeaderEpochFile {
  ...
  public void  newFile(File dir): File = new File(dir, LeaderEpochCheckpointFilename);
}
...
object LeaderEpochCheckpointFile {
  ...
  object Formatter extends CheckpointFileFormatter<EpochEntry> {
    ...
    override public void  toLine(EpochEntry entry): String = s"${entry.epoch} ${entry.startOffset}";

    override public void  fromLine(String line): Option<EpochEntry> = {
...
class LeaderEpochCheckpointFile(val File file) extends LeaderEpochCheckpoint {
  ...
  public void  write(Seq epochs<EpochEntry>): Unit = checkpoint.write(epochs);

  public void  read(): Seq<EpochEntry> = checkpoint.read();
}
