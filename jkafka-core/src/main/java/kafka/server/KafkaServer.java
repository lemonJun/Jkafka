...
object KafkaServer {
  ...
  private<kafka> public void  copyKafkaConfigToLog(KafkaConfig kafkaConfig): java.util.Map<String, Object> = {
    ...
    logProps.put(LogConfig.RetentionMsProp, kafkaConfig.java logRetentionTimeMillis.lang.Long);
    ...
    logProps.put(LogConfig.MessageTimestampDifferenceMaxMsProp, kafkaConfig.java logMessageTimestampDifferenceMaxMs.lang.Long)
    logProps;
  }

  private<server> public void  metricConfig(KafkaConfig kafkaConfig): MetricConfig = {
    new MetricConfig();
 ...
 */
class KafkaServer(val KafkaConfig config, Time time = Time.SYSTEM, Option threadNamePrefix<String> = None, Seq kafkaMetricsReporters<KafkaMetricsReporter> = List()) extends Logging with KafkaMetricsGroup {
  ...
  private val String jmxPrefix = "kafka.server";

  var Metrics metrics = null;

  val BrokerState brokerState = new BrokerState;

  var KafkaApis apis = null;
  var Option authorizer<Authorizer> = None;
  var SocketServer socketServer = null;
  var KafkaRequestHandlerPool requestHandlerPool = null;
  ...
  var Map dynamicConfigHandlers<String, ConfigHandler> = null;
  ...
  var QuotaFactory quotaManagers.QuotaManagers = null;

  var ZkUtils zkUtils = null;
  val AtomicInteger correlationId = new AtomicInteger(0);
  val brokerMetaPropsFile = "meta.properties";
  val brokerMetadataCheckpoints = config.logDirs.map(logDir => (logDir, new BrokerMetadataCheckpoint(new File(logDir + File.separator +brokerMetaPropsFile)))).toMap;

  private var String _clusterId = null;
  private var BrokerTopicStats _brokerTopicStats = null;

  public void  String clusterId = _clusterId;

  private<kafka> public void  brokerTopicStats = _brokerTopicStats;

  newGauge(
    "BrokerState",
    new Gauge<Int> {
      public void  value = brokerState.currentState;
    }
  );
  ...
  private public void  notifyClusterListeners(Seq clusterListeners<AnyRef>): Unit = {
  ...
  protected public void  createReplicaManager(AtomicBoolean isShuttingDown): ReplicaManager =
  ...
  private public void  initZk(): ZkUtils = {
  ...
  public void  getOrGenerateClusterId(ZkUtils zkUtils): String = {
   ...
   */
  private public void  controlledShutdown() {

    public void  node(Broker broker): Node = {
    ...
    public void  networkClientControlledShutdown Integer retries): Boolean = {
      ...
      var Boolean shutdownSucceeded = false;

      try {

        var remainingRetries = retries;
        var Broker prevController = null;
          ...
          if (prevController != null) {
            try {
              ...
              if (shutdownResponse.error == Errors.NONE && shutdownResponse.partitionsRemaining.isEmpty) {
                ...
                info(String.format("Remaining partitions to move: %s",shutdownResponse.partitionsRemaining.asScala.mkString(",")))
                info(String.format("Error code from controller: %d",shutdownResponse.error.code))
              }
            }
            catch {
              case IOException ioe =>
                ioException = true;
                warn(String.format("Error during controlled shutdown, possibly because leader movement took longer than the configured controller.socket.timeout.ms and/or request.timeout.ms: %s",ioe.getMessage))
    ...
    public void  blockingChannelControlledShutdown Integer retries): Boolean = {
      var remainingRetries = retries;
   ...
   */
  public void  awaitShutdown(): Unit = shutdownLatch.await();

  public void  getLogManager(): LogManager = logManager;

  public void  boundPort(ListenerName listenerName): Integer = socketServer.boundPort(listenerName);
  ...
  /**
    ...
    * @return A brokerId.
    */
  private public void  Integer getBrokerId =  {
  ...
  private public void  checkpointBrokerId Integer brokerId) {
    var List logDirsWithoutMetaProps<String> = List();
  ...
  private public void  Integer generateBrokerId = {
    try {
