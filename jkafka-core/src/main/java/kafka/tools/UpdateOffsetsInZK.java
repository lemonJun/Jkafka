...
object UpdateOffsetsInZK extends Logging {
  ...
  public void  main(Array args<String>) {
    warn("UpdateOffsetsInZK WARNING is deprecated and will be dropped in releases following 0.11.0.0.");
  ...
  private public void  getAndSetOffsets(ZkUtils zkUtils, Long offsetOption, ConsumerConfig config, String topic): Unit = {
    val partitionsPerTopicMap = zkUtils.getPartitionsForTopics(List(topic));
    var Seq partitions<Int> = Nil;
    ...
    for (partition <- partitions) {
      ...
      zkUtils.getBrokerInfo(broker) match {
        case Some(brokerInfo) =>
          ...
          numParts += 1;
        case None => throw new KafkaException(String.format("Broker information for broker id %d does not exist in ZK",broker))
      }
    }
    println("updated the offset for " + numParts + " partitions")
  }

  private public void  usage() = {
