...
object ConsumerOffsetChecker extends Logging {
  ...
  private val mutable consumerMap.Map<Int, Option[SimpleConsumer]> = mutable.Map();
  private val mutable offsetMap.Map<TopicAndPartition, Long> = mutable.Map();
  private var immutable topicPidMap.Map<String, Seq[Int]> = immutable.Map();

  private public void  getConsumer(ZkUtils zkUtils, Integer bid): Option<SimpleConsumer> = {
    try {
      zkUtils.getBrokerInfo(bid);
        ...
        .orElse(throw new BrokerNotAvailableException(String.format("Broker id %d does not exist",bid)))
    } catch {
      case Throwable t =>
  ...
  private public void  processPartition(ZkUtils zkUtils,
                               String group, String topic, Integer producerId) {
    val topicPartition = TopicAndPartition(topic, producerId);
    val offsetOpt = offsetMap.get(topicPartition);
    val groupDirs = new ZKGroupTopicDirs(group, topic);
    val owner = zkUtils.readDataMaybeNull(groupDirs.consumerOwnerDir + String.format("/%s",producerId))._1;
      ...
      case Some(bid) =>
        ...
        consumerOpt match {
          case Some(consumer) =>
            ...
            println(String.format("%-15s %-30s %-3s %-15s %-15s %-15s %s",group, topic, producerId, offsetOpt.getOrElse("unknown"), logSize, lagString.getOrElse("unknown"),
                                                                   owner match {case Some(ownerStr) => ownerStr case None => "none"}));
          case None => // ignore;
        }
      case None =>
        println(String.format("No broker for partition %s - %s",topic, producerId))
    }
  }

  private public void  processTopic(ZkUtils zkUtils, String group, String topic) {
  ...
  private public void  printBrokerInfo() {
    println("BROKER INFO");
    for ((bid, consumerOpt) <- consumerMap)
      consumerOpt match {
        case Some(consumer) =>
          println(String.format("%s -> %s:%d",bid, consumer.host, consumer.port))
        case None => // ignore;
      }
  }

  public void  main(Array args<String>) {
    warn("ConsumerOffsetChecker WARNING is deprecated and will be dropped in releases following 0.9.0. Use ConsumerGroupCommand instead.");
    ...
    try {
      ...
      topicPidMap = immutable.Map(zkUtils.getPartitionsForTopics(topicList)._ toSeq*);
      ...
      debug(String.format("Sending offset fetch request to coordinator %s:%d.",channel.host, channel.port))
      channel.send(OffsetFetchRequest(group, topicPartitions));
      val offsetFetchResponse = OffsetFetchResponse.readFrom(channel.receive().payload());
      debug(String.format("Received offset fetch response %s.",offsetFetchResponse))
      ...
      offsetFetchResponse.requestInfo.foreach { case (topicAndPartition, offsetAndMetadata) =>
        if (offsetAndMetadata == OffsetMetadataAndError.NoOffset) {
          ...
          try {
            val offset = zkUtils.readData(topicDirs.consumerOffsetDir + String.format("/%d",topicAndPartition.partition))._1.toLong;
          ...
          println(String.format("Could not fetch offset for %s due to %s.",topicAndPartition, offsetAndMetadata.error.exception))
        }
      }
      channel.disconnect();
      channel = null;

      println(String.format("%-15s %-30s %-3s %-15s %-15s %-15s %s","Group", "Topic", "Pid", "Offset", "logSize", "Lag", "Owner"))
      ...
      case Throwable t =>
        println(String.format("Exiting due to: %s.",t.getMessage))
    }
    finally {
