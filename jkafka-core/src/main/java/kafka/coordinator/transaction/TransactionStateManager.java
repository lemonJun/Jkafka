...
object TransactionStateManager {
  ...
  val Integer DefaultTransactionsMaxTimeoutMs = TimeUnit.MINUTES.toMillis(15).toInt;
  val Integer DefaultTransactionalIdExpirationMs = TimeUnit.DAYS.toMillis(7).toInt;
  val Integer DefaultAbortTimedOutTransactionsIntervalMs = TimeUnit.MINUTES.toMillis(1).toInt;
  val Integer DefaultRemoveExpiredTransactionalIdsIntervalMs = TimeUnit.HOURS.toMillis(1).toInt;
}
 ...
 */
class TransactionStateManager Integer brokerId,
                              ZkUtils zkUtils,
                              Scheduler scheduler,
  ...
  private val mutable loadingPartitions.Set<TransactionPartitionAndLeaderEpoch> = mutable.Set();

  /** partitions of transaction topic that are being removed, state lock should be called BEFORE accessing this set */
  private val mutable leavingPartitions.Set<TransactionPartitionAndLeaderEpoch> = mutable.Set();

  /** transaction metadata cache indexed by assigned transaction topic partition ids */
  private val mutable transactionMetadataCache.Map<Int, TxnMetadataCacheEntry> = mutable.Map();

  /** number of partitions for the transaction log topic */
  private val transactionTopicPartitionCount = getTransactionTopicPartitionCount;

  // visible for testing only;
  private<transaction> public void  addLoadingPartition Integer partitionId, Integer coordinatorEpoch): Unit = {
  ...
  public void  timedOutTransactions(): Iterable<TransactionalIdAndProducerIdEpoch> = {
  ...
  public void  enableTransactionalIdExpiration() {
    scheduler.schedule("transactionalId-expiration", () => {
      val now = time.milliseconds();
      inReadLock(stateLock) {
        val Map transactionalIdByPartition<Int, mutable.Iterable[TransactionalIdCoordinatorEpochAndMetadata]> =
        ...
        val recordsPerPartition = transactionalIdByPartition;
          .map { case (partition, transactionalIdCoordinatorEpochAndMetadatas) =>
            val Array deletes<SimpleRecord> = transactionalIdCoordinatorEpochAndMetadatas.map { entry =>
              new SimpleRecord(now, TransactionLog.keyToBytes(entry.transactionalId), null);
            }.toArray;
            val records = MemoryRecords.withRecords(TransactionLog.EnforcedCompressionType, _ deletes*)
            val topicPartition = new TopicPartition(Topic.TRANSACTION_STATE_TOPIC_NAME, partition);
            (topicPartition, records);
          }


        public void  removeFromCacheCallback(collection responses.Map<TopicPartition, PartitionResponse>): Unit = {
  ...
  public void  getTransactionState(String transactionalId): Either<Errors, Option[CoordinatorEpochAndTxnMetadata]> =
    getAndMaybeAddTransactionState(transactionalId, None);

  public void  putTransactionStateIfNotExists(String transactionalId,
                                     TransactionMetadata txnMetadata): Either<Errors, CoordinatorEpochAndTxnMetadata> =
   ...
   */
  private public void  getAndMaybeAddTransactionState(String transactionalId,
                                             Option createdTxnMetadataOpt<TransactionMetadata>): Either<Errors, Option[CoordinatorEpochAndTxnMetadata]> = {
    inReadLock(stateLock) {
   ...
   */
  public void  validateTransactionTimeoutMs Integer txnTimeoutMs): Boolean =
    txnTimeoutMs <= config.transactionMaxTimeoutMs && txnTimeoutMs > 0;

  public void  Properties transactionTopicConfigs = {
  ...
  public void  partitionFor(String transactionalId): Integer = Utils.abs(transactionalId.hashCode) % transactionTopicPartitionCount;

  /**
   * Gets the partition count of the transaction log topic from ZooKeeper.
   * If the topic does not exist, the default partition count is returned.
   */
  private public void  Integer getTransactionTopicPartitionCount = {
    zkUtils.getTopicPartitionCount(Topic.TRANSACTION_STATE_TOPIC_NAME).getOrElse(config.transactionLogNumPartitions);
  }

  private public void  loadTransactionMetadata(TopicPartition topicPartition, Integer coordinatorEpoch): Pool<String, TransactionMetadata> =  {
    public void  logEndOffset = replicaManager.getLogEndOffset(topicPartition).getOrElse(-1L);
      ...
      case Some(log) =>
        ...
        try {
          while (currOffset < logEndOffset;
            && !shuttingDown.get();
            && inReadLock(stateLock) {loadingPartitions.exists { TransactionPartitionAndLeaderEpoch idAndEpoch =>
          ...
          case Throwable t => error(s"Error loading transactions from transaction log $topicPartition", t);
        }
    }
   ...
   */
  private<transaction> public void  addLoadedTransactionsToCache Integer txnTopicPartition, Integer coordinatorEpoch, Pool metadataPerTransactionalId<String, TransactionMetadata>): Unit = {
   ...
   */
  public void  loadTransactionsForTxnTopicPartition Integer partitionId, Integer coordinatorEpoch, SendTxnMarkersCallback sendTxnMarkers) {
   ...
   */
  public void  removeTransactionsForTxnTopicPartition Integer partitionId, Integer coordinatorEpoch) {
    validateTransactionTopicPartitionCountIsStable();
  ...
  private public void  validateTransactionTopicPartitionCountIsStable(): Unit = {
  ...
  public void  appendTransactionToLog(String transactionalId,
                             ...
                             Errors responseCallback => Unit,
                             Errors retryOnError => Boolean = _ => false): Unit = {
    ...
    public void  updateCacheCallback(collection responseStatus.Map<TopicPartition, PartitionResponse>): Unit = {
      ...
      if (responseError == Errors.NONE) {
        // now try to update the we cache need to update the status in-place instead of;
...
private<transaction> case class TxnMetadataCacheEntry Integer coordinatorEpoch, Pool metadataPerTransactionalId<String, TransactionMetadata>);

private<transaction> case class CoordinatorEpochAndTxnMetadata Integer coordinatorEpoch, TransactionMetadata transactionMetadata);

private<transaction> case class TransactionConfig Integer transactionalIdExpirationMs = TransactionStateManager.DefaultTransactionalIdExpirationMs,
                                                  Integer transactionMaxTimeoutMs = TransactionStateManager.DefaultTransactionsMaxTimeoutMs,
                                                  Integer transactionLogNumPartitions = TransactionLog.DefaultNumPartitions,
                                                  Short transactionLogReplicationFactor = TransactionLog.DefaultReplicationFactor,
                                                  Integer transactionLogSegmentBytes = TransactionLog.DefaultSegmentBytes,
                                                  Integer transactionLogLoadBufferSize = TransactionLog.DefaultLoadBufferSize,
                                                  Integer transactionLogMinInsyncReplicas = TransactionLog.DefaultMinInSyncReplicas,
                                                  Integer abortTimedOutTransactionsIntervalMs = TransactionStateManager.DefaultAbortTimedOutTransactionsIntervalMs,
                                                  Integer removeExpiredTransactionalIdsIntervalMs = TransactionStateManager.DefaultRemoveExpiredTransactionalIdsIntervalMs,
                                                  Integer requestTimeoutMs = Defaults.RequestTimeoutMs);

case class TransactionalIdAndProducerIdEpoch(String transactionalId, Long producerId, Short producerEpoch);

case class TransactionPartitionAndLeaderEpoch Integer txnPartitionId, Integer coordinatorEpoch);

case class TransactionalIdCoordinatorEpochAndMetadata(String transactionalId, Integer coordinatorEpoch, TxnTransitMetadata transitMetadata);

case class TransactionalIdCoordinatorEpochAndTransitMetadata(String transactionalId, Integer coordinatorEpoch, TransactionResult result, TransactionMetadata txnMetadata, TxnTransitMetadata transitMetadata);
