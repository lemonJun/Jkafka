...
object ReplicaVerificationTool extends Logging {
  ...
  val dateFormatString = "yyyy-MM-dd mm HH:ss,SSS";
  ...
  public void  main(Array args<String>): Unit = {
   ...
   if(args.length == 0)
    ...
    val Seq topicPartitionReplicaList<TopicPartitionReplica> = filteredTopicMetadata.flatMap(
    ...
    val Map topicAndPartitionsPerBroker<Int, Seq[TopicAndPartition]> = topicPartitionReplicaList.groupBy(_.replicaId);
    ...
    val Map expectedReplicasPerTopicAndPartition<TopicAndPartition, Int> =
    ...
    val Map leadersPerBroker<Int, Seq[TopicAndPartition]> = filteredTopicMetadata.flatMap { topicMetadataResponse =>
    ...
    val Iterable fetcherThreads<ReplicaFetcher> = topicAndPartitionsPerBroker.map {
...
private case class TopicPartitionReplica(String topic,  Integer partitionId,  Integer replicaId);

private case class MessageInfo Integer replicaId, Long offset, Long nextOffset, Long checksum);

private class ReplicaBuffer(Map expectedReplicasPerTopicAndPartition<TopicAndPartition, Int>,
                            Map leadersPerBroker<Int, Seq[TopicAndPartition]>,
                            Integer expectedNumFetchers,
                            Map brokerMap<Int, BrokerEndPoint>,
                            Long initialOffsetTime,
                            Long reportInterval) extends Logging {
  ...
  private var Long maxLag = -1L;
  private var Long offsetWithMaxLag = -1L;
  private var TopicAndPartition maxLagTopicAndPartition = null;
  initialize();
  ...
  public void  getFetcherBarrier() = fetcherBarrier.get();

  public void  createNewVerificationBarrier() {
    verificationBarrier.set(new CountDownLatch(1))
  }

  public void  getVerificationBarrier() = verificationBarrier.get()
  ...
  private public void  offsetResponseStringWithError(OffsetResponse offsetResponse): String = {
    offsetResponse.partitionErrorAndOffsets.filter { case (_, partitionOffsetsResponse) =>
      partitionOffsetsResponse.error != Errors.NONE;
    }.mkString;
  }

  private public void  setInitialOffsets() {
    for ((brokerId, topicAndPartitions) <- leadersPerBroker) {
      ...
      val Map initialOffsetMap<TopicAndPartition, PartitionOffsetRequestInfo> =
  ...
  public void  addFetchedData(TopicAndPartition topicAndPartition, Integer replicaId, FetchResponsePartitionData partitionData) {
    messageSetCache.get(topicAndPartition).put(replicaId, partitionData);
  }

  public void  getOffset(TopicAndPartition topicAndPartition) = {
    fetchOffsetMap.get(topicAndPartition);
  }

  public void  verifyCheckSum(String println => Unit) {
    ...
    for ((topicAndPartition, fetchResponsePerReplica) <- messageSetCache) {
      ...
      while (isMessageInAllReplicas) {
        var Option messageInfoFromFirstReplicaOpt<MessageInfo> = None;
...
private class ReplicaFetcher(String name, BrokerEndPoint sourceBroker, Iterable topicAndPartitions<TopicAndPartition>,
                             ReplicaBuffer replicaBuffer, Integer socketTimeout, Integer socketBufferSize,
                             Integer fetchSize, Integer maxWait, Integer minBytes, Boolean doVerification)
  ...
  override public void  doWork() {
    ...
    var FetchResponse response = null;
    try {
