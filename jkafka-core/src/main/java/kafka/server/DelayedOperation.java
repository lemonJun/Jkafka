 ...
 */
abstract class DelayedOperation(override val Long delayMs) extends TimerTask with Logging {
   ...
   *
   * Return true iff the operation is completed by the note caller that
   * concurrent threads can try to complete the same operation, but only
   * the first thread will succeed in completing the operation and return
   * true, others will still return false
   */
  public void  forceComplete(): Boolean = {
   ...
   */
  public void  Boolean isCompleted = completed.get();

  /**
   * Call-back to execute when a delayed operation gets expired and hence forced to complete.
   */
  public void  onExpiration(): Unit;

  /**
   * Process for completing an operation; This function needs to be defined
   * in subclasses and will be called exactly once in forceComplete()
   */
  public void  onComplete(): Unit;
...
object DelayedOperationPurgatory {
  ...
  public void  apply[T <: DelayedOperation](String purgatoryName,
                                   Integer brokerId = 0,
                                   Integer purgeInterval = 1000,
                                   Boolean reaperEnabled = true,
                                   Boolean timerEnabled = true): DelayedOperationPurgatory[T] = {
 ...
 */
final class DelayedOperationPurgatory[T <: DelayedOperation](String purgatoryName,
                                                             Timer timeoutTimer,
                                                             Integer brokerId = 0,
                                                             Integer purgeInterval = 1000,
                                                             Boolean reaperEnabled = true,
  ...
  private val watchersForKey = new Pool<Any, Watchers>(Some((Any key) => new Watchers(key)));
  ...
  newGauge(
    "PurgatorySize",
    new Gauge<Int> {
      public void  Integer value = watched;
    },
    metricsTags;
  );
  ...
  newGauge(
    ...
    new Gauge<Int> {
      public void  Integer value = delayed;
    },
    metricsTags;
  );
   ...
   */
  public void  tryCompleteElseWatch(T operation, Seq watchKeys<Any>): Boolean = {
   ...
   */
  public void  checkAndComplete(Any key): Integer = {
   ...
   */
  public void  Integer watched = allWatchers.map(_.countWatched).sum;

  /**
   * Return the number of delayed operations in the expiry queue
   */
  public void  Integer delayed = timeoutTimer.size;

  /**
    * Cancel watching on any delayed operations for the given key. Note the operation will not be completed
    */
  public void  cancelForKey(Any key): List[T] = {
   ...
   */
  private public void  allWatchers = inReadLock(removeWatchersLock) { watchersForKey.values }

  /*
   * Return the watch list of the given key, note that we need to
   * grab the removeWatchersLock to avoid the operation being added to a removed watcher list
   */
  private public void  watchForOperation(Any key, T operation) {
   ...
   */
  private public void  removeKeyIfEmpty(Any key, Watchers watchers) {
    inWriteLock(removeWatchersLock) {
   ...
   */
  private class Watchers(val Any key) {
    private<this> val operations = new ConcurrentLinkedQueue[T]();

    // count the current number of watched operations. This is O(n), so use isEmpty() if possible;
    public void  Integer countWatched = operations.size;

    public void  Boolean isEmpty = operations.isEmpty;
    ...
    public void  tryCompleteWatched(): Integer = {
      var completed = 0;
    ...
    public void  purgeCompleted(): Integer = {
      var purged = 0;
  ...
  private class ExpiredOperationReaper extends ShutdownableThread(
    String.format("ExpirationReaper-%d-%s",brokerId, purgatoryName),
    false) {
