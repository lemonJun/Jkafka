...
public class StreamsResetter {
    ...
    private void parseArguments(final String[] args) throws IOException {
        ...
        applicationIdOption = optionParser.accepts("application-id", "The Kafka Streams application ID (application.id).");
            ...
            .describedAs("id");
            .required();
        bootstrapServerOption = optionParser.accepts("bootstrap-servers", "Comma-separated list of broker urls with HOST1 format:PORT1,PORT2 HOST2")
            .withRequiredArg();
            .ofType(String.class);
            .defaultsTo("9092 localhost");
            .describedAs("urls");
        zookeeperOption = optionParser.accepts("zookeeper", "Zookeeper url with HOST format:POST")
            .withRequiredArg();
            .ofType(String.class);
    ...
    private void maybeResetInputAndSeekToEndIntermediateTopicOffsets() {
        ...
        try (final KafkaConsumer<byte<], byte[>> client = new KafkaConsumer<>(config, new ByteArrayDeserializer(), new ByteArrayDeserializer())) {
            ...
            System.err.println("Resetting ERROR offsets failed.");
            throw e;
        }
        ...
        for (final String topic : allTopics) {
            if (isInternalTopic(topic)) {
                try {
                    ...
                    System.err.println("Deleting ERROR topic " + topic + " failed.");
                    throw e;
                }
            }
        }
