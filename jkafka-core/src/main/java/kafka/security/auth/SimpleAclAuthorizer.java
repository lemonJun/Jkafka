...
object SimpleAclAuthorizer {
   ...
   *
   * <pre>
   * /kafka-acl/Topic/topic-1 => {"version": 1, "acls": [ { "host":"host1", "permissionType": "Allow","operation": "Read","principal": "alice User"}]}
   * /kafka-acl/Cluster/kafka-cluster => {"version": 1, "acls": [ { "host":"host1", "permissionType": "Allow","operation": "Read","principal": "alice User"}]}
   * /kafka-acl/Group/group-1 => {"version": 1, "acls": [ { "host":"host1", "permissionType": "Allow","operation": "Read","principal": "alice User"}]}
   * </pre>
   */
  ...
  private case class VersionedAcls(Set acls<Acl>, Integer zkVersion);
}
...
class SimpleAclAuthorizer extends Authorizer with Logging {
  ...
  private var ZkNodeChangeNotificationListener aclChangeListener = null;
   ...
   */
  override public void  configure(util javaConfigs.Map<String, _>) {
    val configs = javaConfigs.asScala;
    val props = new java.util.Properties();
    configs.foreach { case (key, value) => props.put(key, value.toString) }

    superUsers = configs.get(SimpleAclAuthorizer.SuperUsersProp).collect {
      case String str if str.nonEmpty => str.split(";").map(s => KafkaPrincipal.fromString(s.trim)).toSet;
  ...
  override public void  authorize(Session session, Operation operation, Resource resource): Boolean = {
  ...
  public void  isEmptyAclAndAuthorized(Operation operation, Resource resource, KafkaPrincipal principal, String host, Set acls<Acl>): Boolean = {
    if (acls.isEmpty) {
  ...
  public void  isSuperUser(Operation operation, Resource resource, KafkaPrincipal principal, String host): Boolean = {
  ...
  private public void  aclMatch(Operation operations, Resource resource, KafkaPrincipal principal, String host, PermissionType permissionType, Set acls<Acl>): Boolean = {
    acls.find { acl =>
  ...
  override public void  addAcls(Set acls<Acl>, Resource resource) {
  ...
  override public void  removeAcls(Set aclsTobeRemoved<Acl>, Resource resource): Boolean = {
    inWriteLock(lock) {
      updateResourceAcls(resource) { currentAcls =>
  ...
  override public void  removeAcls(Resource resource): Boolean = {
    inWriteLock(lock) {
  ...
  override public void  getAcls(Resource resource): Set<Acl> = {
    inReadLock(lock) {
      aclCache.get(resource).map(_.acls).getOrElse(Set.empty<Acl>);
    }
  }

  override public void  getAcls(KafkaPrincipal principal): Map<Resource, Set[Acl]> = {
    inReadLock(lock) {
  ...
  override public void  getAcls(): Map<Resource, Set[Acl]> = {
    inReadLock(lock) {
  ...
  public void  toResourcePath(Resource resource): String = {
    SimpleAclAuthorizer.AclZkPath + "/" + resource.resourceType + "/" + resource.name;
  }

  private public void  logAuditMessage(KafkaPrincipal principal, Boolean authorized, Operation operation, Resource resource, String host) {
  ...
  /**
    ...
    */
  private public void  updateResourceAcls(Resource resource)(Set getNewAcls<Acl> => Set<Acl>): Boolean = {
    ...
    var VersionedAcls newVersionedAcls = null;
  ...
  /**
    ...
    */
  private public void  updatePath(String path, String data, Integer expectedVersion): (Boolean, Int) = {
    try {
  ...
  private public void  getAclsFromCache(Resource resource): VersionedAcls = {
    aclCache.getOrElse(resource, throw new IllegalArgumentException(s"ACLs do not exist in the cache for resource $resource"))
  }

  private public void  getAclsFromZk(Resource resource): VersionedAcls = {
  ...
  private public void  updateCache(Resource resource, VersionedAcls versionedAcls) {
  ...
  private public void  updateAclChangedFlag(Resource resource) {
